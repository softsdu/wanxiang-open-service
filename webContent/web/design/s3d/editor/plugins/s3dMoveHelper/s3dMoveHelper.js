import { Raycaster, BufferGeometry, Float32BufferAttribute, Line, LineBasicMaterial, PlaneGeometry, MeshLambertMaterial, Mesh, Box3, Vector3, Vector2 } from '../../node_modules/three/build/three.module.js';
import { TransformControls } from '../../commonjs/threejs/custom/TransformControls.js';
import { s3dTransformMode, s3dLayerType, s3dOperateType, cmnPcr } from '../../commonjs/common/common.js';
import './s3dMoveHelper.css.js';
...
let S3dMoveHelper = function () {
  ...
  this.removeEventFunction = function (eventName, func) {
    ...
    if (allFuncs != null) {
      ...
      thatS3dMoveHelper.eventFunctions[eventName] = newAllFuncs;
    }
  };
  ...
  this.init = function (p) {
    ...
    //绑定事件
    if (p.config.onObject3DPosRotScaleChanged != null) {
      thatS3dMoveHelper.addEventFunction("onObject3DPosRotScaleChanged", p.config.onObject3DPosRotScaleChanged);
    }
    ...
    if (p.config.afterObject3DPosRotScaleChanged != null) {
      thatS3dMoveHelper.addEventFunction("afterObject3DPosRotScaleChanged", p.config.afterObject3DPosRotScaleChanged);
    }
  };
  ...
  this.setMode = function (mode) {
    switch (mode) {
      case s3dTransformMode.position:
        {
          thatS3dMoveHelper.transformControl.setMode("translate");
          break;
        }
      case s3dTransformMode.rotation:
        {
          thatS3dMoveHelper.transformControl.setMode("rotate");
          break;
        }
      case s3dTransformMode.scale:
      default:
        {
          thatS3dMoveHelper.transformControl.setMode("scale");
          break;
        }
    }
  };
  this.getMode = function () {
    let mode = null;
    let controlMode = thatS3dMoveHelper.transformControl.getMode();
    switch (controlMode) {
      case "translate":
        {
          mode = s3dTransformMode.position;
          break;
        }
      case "rotate":
        {
          mode = s3dTransformMode.rotation;
          break;
        }
      case "scale":
        {
          mode = s3dTransformMode.scale;
          break;
        }
    }
    return mode;
  ...
  this.autoRefreshAttachDistance = function () {
    let leftPosition = new Vector2(-0.99, 0);
    let rightPosition = new Vector2(0.99, 0);
    ...
    if (leftIntersectPoint != null && rightIntersectPoint != null) {
      let viewWidth = Math.sqrt((rightIntersectPoint.x - leftIntersectPoint.x) * (rightIntersectPoint.x - leftIntersectPoint.x) + (rightIntersectPoint.y - leftIntersectPoint.y) * (rightIntersectPoint.y - leftIntersectPoint.y) + (rightIntersectPoint.z - leftIntersectPoint.z) * (rightIntersectPoint.z - leftIntersectPoint.z));
      //更改吸附距离
  ...
  this.getGroundPlaneRaycasterPoint = function (position) {
    let distanceRaycaster = new Raycaster();
  ...
  this.initControl = function (p) {
    ...
    thatS3dMoveHelper.manager.viewer.disableObjectLayer(control, s3dLayerType.viewLayer, true);
    thatS3dMoveHelper.manager.viewer.enableObjectLayer(control, s3dLayerType.editLayer, true);
    thatS3dMoveHelper.manager.viewer.enableObjectLayer(control, s3dLayerType.controlLayer, true);
    let raycaster = control.getRaycaster();
    thatS3dMoveHelper.manager.viewer.enableObjectLayer(raycaster, s3dLayerType.editLayer, true);
    thatS3dMoveHelper.manager.viewer.enableObjectLayer(raycaster, s3dLayerType.controlLayer, true);
  ...
  this.initAttachHelpLines = function () {
    ...
    let axisInfo = thatS3dMoveHelper.manager.s3dObject.axis;
    let geometryX = new BufferGeometry();
    let positionsX = [];
    positionsX.push(0, thatS3dMoveHelper.gridYZero, -axisInfo.size.z * 5 / 4);
    positionsX.push(0, thatS3dMoveHelper.gridYZero, axisInfo.size.z * 5 / 4);
    geometryX.setAttribute("position", new Float32BufferAttribute(positionsX, 3));
    let lineX = new Line(geometryX, new LineBasicMaterial({
    ...
    let geometryZ = new BufferGeometry();
    let positionsZ = [];
    positionsZ.push(-axisInfo.size.x * 5 / 4, thatS3dMoveHelper.gridYZero, 0);
    positionsZ.push(axisInfo.size.x * 5 / 4, thatS3dMoveHelper.gridYZero, 0);
    geometryZ.setAttribute("position", new Float32BufferAttribute(positionsZ, 3));
    let lineZ = new Line(geometryZ, new LineBasicMaterial({
      color: thatS3dMoveHelper.attachHelpLines.color,
      transparent: true,
      opacity: thatS3dMoveHelper.attachHelpLines.opacity,
      depthTest: false
    ...
    let xCenter = 0;
    let zCenter = 0;
    let planeGeometryY = new PlaneGeometry(sizeX, sizeZ);
    let planeMaterialY = new MeshLambertMaterial({
      color: thatS3dMoveHelper.attachHelpLines.color,
      transparent: true,
      opacity: thatS3dMoveHelper.attachHelpLines.opacity
    });
    let planeY = new Mesh(planeGeometryY, planeMaterialY);
  ...
  //绑定控制器
  this.attach = function (objectJArray) {
    if (objectJArray == null || objectJArray.length === 0 || objectJArray.length > 1) {
      thatS3dMoveHelper.detach();
    } else {
      thatS3dMoveHelper.attachLines = null;
      let objectJson = objectJArray[0];
      let object3D = thatS3dMoveHelper.manager.viewer.getObject3DById(objectJson.id);
      thatS3dMoveHelper.currentObject3D = object3D;
      if (!object3D.userData.info.useWorldPosition) {
  ...
  this.transformObjectInSilence = function (nodeJsons) {
    for (let i = 0; i < nodeJsons.length; i++) {
      ...
      object3D.scale.set(nodeJson.scale[0], nodeJson.scale[1], nodeJson.scale[2]);
      thatS3dMoveHelper.manager.viewer.updateHelper(object3D.userData.info.id);
    }
  };
  ...
  this.beginAddToUndoList = function (nodeId) {
    let nodeJsons = [];
    nodeJsons.push(thatS3dMoveHelper.manager.viewer.cloneJsonById(nodeId));
  ...
  this.calcNearestCenterValue = function (valueArray, minValue, centerValue, maxValue) {
    ...
    return {
      newValue: newCenterValue,
      helpLineValue: newCenterValue == null ? null : helpLineValue
    };
  };
  ...
  this.transformMouseUp = function (upEv) {
    if (thatS3dMoveHelper.dragging) {
      ...
      thatS3dMoveHelper.afterObject3DPosRotScaleChanged({
        object3D: object3D
      });
    }
  ...
  this.transformDrag = function (transformEv) {
    ...
    thatS3dMoveHelper.getOtherPropertyValues(object3D);
    	...
    	if(thatS3dMoveHelper.onDragTransformControl({
    		...
    		if(transformEv.target.axis === "X"){
    			...
    			if(attachValueObj.newValue != null){
    				thatS3dMoveHelper.updateObject3DPosition(object3D, attachValueObj.newValue, object3D.position.y, object3D.position.z);
    			}
    			...
    			if(attachValueObj.newValue != null){
    				thatS3dMoveHelper.updateObject3DPosition(object3D, object3D.position.x, attachValueObj.newValue, object3D.position.z);
    			}
    			...
    			if(attachValueObj.newValue != null){
    				thatS3dMoveHelper.updateObject3DPosition(object3D, object3D.position.x, object3D.position.y, attachValueObj.newValue);
    			}   
    			...
    			if(attachValueObjX.newValue != null && attachValueObjZ.newValue != null){
    				thatS3dMoveHelper.updateObject3DPosition(object3D, attachValueObjX.newValue, object3D.position.y, attachValueObjZ.newValue);
    				thatS3dMoveHelper.refreshAttachHelpLine();
    			}
    			else {
    				if (attachValueObjX.newValue != null) {
    					thatS3dMoveHelper.updateObject3DPosition(object3D, attachValueObjX.newValue, object3D.position.y, object3D.position.z);
    				}
    				if (attachValueObjZ.newValue != null) {
    					thatS3dMoveHelper.updateObject3DPosition(object3D, object3D.position.x, object3D.position.y, attachValueObjZ.newValue);
    				}
    			}
    			thatS3dMoveHelper.refreshAttachHelpLine({
    ...
    */
    ...
    info.position = [newPosition[0], newPosition[1], newPosition[2]];
    thatS3dMoveHelper.manager.viewer.updateHelper(info.id);

    //取消显示辅助线
    //thatS3dMoveHelper.refreshAttachHelpLine2d();

    thatS3dMoveHelper.onObject3DPosRotScaleChanged({
      object3D: object3D
    });
  };
  //更新位置信息
  this.updateObject3DPosition = function (object3D, posX, posY, posZ) {
    let x = posX;
    let y = posY;
    let z = posZ;
  ...
  this.onObject3DPosRotScaleChanged = function (p) {
    thatS3dMoveHelper.doEventFunction("onObject3DPosRotScaleChanged", {
      objectJson: thatS3dMoveHelper.manager.viewer.getNodeJson(p.object3D.userData.info.id),
      mode: thatS3dMoveHelper.getMode()
    });
  };

  //当构件对象位置变化后触发事件 added by ls 20220920
  this.afterObject3DPosRotScaleChanged = function (p) {
    thatS3dMoveHelper.doEventFunction("afterObject3DPosRotScaleChanged", {
      objectJson: thatS3dMoveHelper.manager.viewer.getNodeJson(p.object3D.userData.info.id),
      mode: thatS3dMoveHelper.getMode()
    });
  };
  ...
  this.onDragTransformControl = function (p) {
    return thatS3dMoveHelper.doEventFunctionWithResult("onDragTransformControl", {
      nodeId: p.object3D.userData.info.id,
  ...
  this.get2DPosition = function (vec3d) {
    let vec = new Vector3(vec3d.x, vec3d.y, vec3d.z);
  ...
  this.refreshAttachHelpLine2d = function () {
    ...
    if (thatS3dMoveHelper.currentObject3D != null) {
      ...
      let posXPA = thatS3dMoveHelper.get2DPosition(new Vector3(0, 0, propertyValues.minZ));
      let posXPB = thatS3dMoveHelper.get2DPosition(new Vector3(propertyValues.minX, 0, propertyValues.minZ));
      ...
      let posZPA = thatS3dMoveHelper.get2DPosition(new Vector3(propertyValues.minX, 0, 0));
      let posZPB = thatS3dMoveHelper.get2DPosition(new Vector3(propertyValues.minX, 0, propertyValues.minZ));
      ...
      let lenXPA = thatS3dMoveHelper.get2DPosition(new Vector3(propertyValues.minX, 0, propertyValues.minZ));
      let lenXPB = thatS3dMoveHelper.get2DPosition(new Vector3(propertyValues.maxX, 0, propertyValues.minZ));
      ...
      let lenZPA = thatS3dMoveHelper.get2DPosition(new Vector3(propertyValues.minX, 0, propertyValues.minZ));
      let lenZPB = thatS3dMoveHelper.get2DPosition(new Vector3(propertyValues.minX, 0, propertyValues.maxZ));
      ...
      let maxXPA = thatS3dMoveHelper.get2DPosition(new Vector3(propertyValues.minX, 0, propertyValues.maxZ));
      let maxXPB = thatS3dMoveHelper.get2DPosition(new Vector3(propertyValues.maxX, 0, propertyValues.maxZ));
      ...
      let maxZPA = thatS3dMoveHelper.get2DPosition(new Vector3(propertyValues.maxX, 0, propertyValues.minZ));
      let maxZPB = thatS3dMoveHelper.get2DPosition(new Vector3(propertyValues.maxX, 0, propertyValues.maxZ));
      ...
      $(posXText).css({
        display: posXLineLen > textRectWidth ? "block" : "none"
      });
      ...
      $(posZText).css({
        display: posZLineLen > textRectWidth ? "block" : "none"
      });
      ...
      $(lenXText).css({
        display: lenXLineLen > textRectWidth ? "block" : "none"
      });
      ...
      $(lenZText).css({
        display: lenZLineLen > textRectWidth ? "block" : "none"
      });
  ...
  this.getDisplayValueStr = function (sourceValue, fixNum) {
    ...
    return cmnPcr.decimalToStr(common3DFunction.s2v(sourceValue, thatS3dMoveHelper.manager.viewer.distanceRatio), false, fixNum, true);
  };
  ...
  this.initAttachHelpLine2dHtml = function (containerElement) {
    ...
    $("#" + thatS3dMoveHelper.containerId).find(".popPosSettingInput").keydown(function (ev) {
      switch (ev.keyCode) {
        case 13:
          {
            if (thatS3dMoveHelper.selectedObjectObject3D != null) {
              if (thatS3dMoveHelper.objectPopChangePos()) {
                thatS3dMoveHelper.closePopPosEditor();
              }
  ...
  this.refreshDrawHelpArc2D = function (fromPoint, crossPoint, toPoint) {
    if (fromPoint != null && crossPoint != null) {
      ...
      thatS3dMoveHelper.get2DPosition(new Vector3(fromPoint.x, fromPoint.y, fromPoint.z));
      let crossPoint2D = thatS3dMoveHelper.get2DPosition(new Vector3(crossPoint.x, crossPoint.y, crossPoint.z));
      thatS3dMoveHelper.get2DPosition(new Vector3(toPoint.x, toPoint.y, toPoint.z));
      let arc2DRect = $("#" + thatS3dMoveHelper.containerId).find(".drawHelpArcRect[name='arcDegree']")[0];
      $(arc2DRect).css({
        display: Math.round(angleDegree) > 0 ? "block" : "none"
      });
  ...
  this.refreshDrawHelpLine2D = function (fromPoint, toPoint) {
    if (fromPoint != null) {
      let textRectWidth = 40;
      let textRectHeight = 18;
      let fromPoint2D = thatS3dMoveHelper.get2DPosition(new Vector3(fromPoint.x, fromPoint.y, fromPoint.z));
      let toPoint2D = thatS3dMoveHelper.get2DPosition(new Vector3(toPoint.x, toPoint.y, toPoint.z));
      ...
      $(lineLengthText).css({
        display: line2DLen > textRectWidth ? "block" : "none"
      });
...
export { S3dMoveHelper as default };
