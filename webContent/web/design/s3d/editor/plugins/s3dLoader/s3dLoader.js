...
let S3dLoader = function () {
  ...
  this.init = function (p) {
    ...
    if (p.config.systemAfterLoadS3dFile != null) {
      thatS3dLoader.addEventFunction("afterLoadS3dFile", p.config.systemAfterLoadS3dFile);
    }
    thatS3dLoader.show();
  ...
  this.getObjectMapJsonUrl = function (path, objectFileName) {
    return path + objectFileName;
  };
  this.getObjectTypeMapJsonUrl = function (path, objectTypeFileName) {
    return path + objectTypeFileName;
  };
  ...
  this.hide = function (p) {
    $("#" + thatS3dLoader.containerId).find(".s3dLoaderStatusContainer").fadeOut(100);
  };

  //获取html
  this.getWinHtml = function () {
    return "<div class=\"s3dLoaderStatusContainer\"></div>";
  };
