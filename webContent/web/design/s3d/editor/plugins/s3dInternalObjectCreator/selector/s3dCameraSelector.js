...
let S3dCameraSelector = function () {
  //当前对象
  const thatCameraSelector = this;
  ...
  this.addEventFunction = function (eventName, func) {
    let allFuncs = thatCameraSelector.eventFunctions[eventName];
    if (allFuncs == null) {
      allFuncs = [];
      thatCameraSelector.eventFunctions[eventName] = allFuncs;
    }
    allFuncs.push(func);
  };
  ...
  this.init = function (p) {
    thatCameraSelector.containerId = p.containerId;
    thatCameraSelector.manager = p.manager;
  };

  //隐藏
  this.hide = function () {
    thatCameraSelector.selectorWindow.hide();
  };

  //隐藏
  this.show = function (p) {
    thatCameraSelector.groupId = p.groupId;
    if (thatCameraSelector.selectorWindow == null) {
      thatCameraSelector.initWindow();
      thatCameraSelector.initComponentList();
    } else {
      thatCameraSelector.selectorWindow.show();
    }
  };

  //获取显示状态
  this.getVisible = function () {
    return $("#" + thatCameraSelector.containerId).find(".s3dInternalObjectCreatorSelectorContainer").css("display") === "block";
  };
  ...
  this.initWindow = function () {
    let popContainer = new PopupContainer({
      ...
      top: 50,
      canClose: false,
      title: thatCameraSelector.title,
      containerId: thatCameraSelector.containerId
    });
    popContainer.show();
    let innerHtml = thatCameraSelector.getHtml();
    $("#" + popContainer.contentId).html(innerHtml);
    thatCameraSelector.selectorWindow = popContainer;
  };

  //构造html
  this.getHtml = function () {
    return "<div class=\"s3dInternalObjectCreatorSelectorContainer\">" + "<div class=\"s3dInternalObjectCreatorSelectorListContainer\" name='camera'></div>" + "</div>";
  };

  //获取构件列表
  this.initComponentList = function (p) {
    let componentListHtml = thatCameraSelector.getComponentListHtml(cameraComponentList);
    let cameraContainer = $("#" + thatCameraSelector.containerId).find(".s3dInternalObjectCreatorSelectorListContainer[name='camera']");
    $(cameraContainer).html(componentListHtml);
    $(cameraContainer)[0].scrollTop = 0;
    $(cameraContainer).find(".s3dInternalObjectCreatorSelectorComponentContainer").click(function () {
      ...
      thatCameraSelector.addComponent({
      ...
      thatCameraSelector.hide();
    });
  };

  //插入构件到当前组
  this.addComponent = function (p) {
    thatCameraSelector.manager.viewer.cancelSelectObject3Ds();
    thatCameraSelector.manager.viewer.addNewInternalObject({
      position: null,
      rotation: null,
      scale: null,
      ...
      parentId: thatCameraSelector.groupId,
      parameters: {},
      isServer: false,
      isLocal: false,
      isInternal: true,
      type: s3dElement3DType.camera
    });
  };
  ...
  this.getComponentListHtml = function (componentJArray) {
    ...
    if (componentJArray.length === 0) {
      ...
      for (let i = 0; i < componentJArray.length; i++) {
        ...
        html += thatCameraSelector.getComponentItemHtml(componentJson);
      }
    }
    return html;
  };
  ...
  this.getComponentItemHtml = function (componentJson) {
    ...
    let imgUrl = componentJson.image == null ? null : thatCameraSelector.manager.layout.imagesFolder + "internal/camera/" + componentJson.image;
...
export { S3dCameraSelector as default };
