import { MeshStandardMaterial, FrontSide, LineBasicMaterial, PlaneGeometry, Mesh, BufferGeometry, Float32BufferAttribute, LineSegments, Box3, Object3D } from '../../node_modules/three/build/three.module.js';
import { FontLoader } from '../../node_modules/three/examples/jsm/loaders/FontLoader.js';
import { TextGeometry } from '../../node_modules/three/examples/jsm/geometries/TextGeometry.js';
import { s3dLayerType, cmnPcr } from '../../commonjs/common/common.js';
...
let S3dAxis = function () {
  ...
  //y方向（地坪在y轴位置）
  this.groundZero = 0; //-0.001;

  //忽略误差的值（有时候double运算会出现不精确的问题）
  this.ignoreSize = 0.0001;

  //网格的起始刻度，为了解决plane和grid覆盖显示的问题
  this.yZero = 0; //0.0001;
  ...
  this.initMaterials = function () {
    thatS3dAxis.planeMaterial = new MeshStandardMaterial({
    ...
    //刻度字体材质
    thatS3dAxis.textMaterial = new MeshStandardMaterial({
      color: thatS3dAxis.textColor
    });

    //网格材质
    thatS3dAxis.gridMaterial = new LineBasicMaterial({
      color: thatS3dAxis.gridColor
    });

    //边框材质
    thatS3dAxis.borderMaterial = new LineBasicMaterial({
      color: thatS3dAxis.borderColor
    });

    //x轴网材质
    thatS3dAxis.xMaterial = new LineBasicMaterial({
      color: thatS3dAxis.xColor
    });

    //z轴网材质
    thatS3dAxis.zMaterial = new LineBasicMaterial({
  ...
  //隐藏
  this.hide = function () {
    $("#" + thatS3dAxis.containerId).find(".s3dPropertyEditorContainer").css({
      "display": "none"
    });
  };
  ...
  this.loadFont = function (p) {
    ...
    let fontUrl = thatS3dAxis.manager.layout.fontsFolder + "helvetiker_bold.typeface.json";
    textLoad.load(fontUrl, function (font) {
  ...
  this.addGroundPlane = function () {
    ...
    let plane = new Mesh(planeGeometry, thatS3dAxis.planeMaterial);
    plane.rotation.x = -0.5 * Math.PI;
    plane.position.set(xCenter, thatS3dAxis.groundZero, zCenter);
    plane.receiveShadow = true;
    plane.isGroundPlane = true;
    plane.renderOrder = -1;
    thatS3dAxis.manager.viewer.disableObjectLayer(plane, s3dLayerType.viewLayer, true);
    thatS3dAxis.manager.viewer.enableObjectLayer(plane, s3dLayerType.editLayer, true);
  ...
  this.addAxisLines = function (lineInfos, direction, textParameter) {
    ...
    for (let i = 0; i < lineInfos.length; i++) {
      ...
      let line = new LineSegments(geometry, lineInfo.material);
      line.isAxis = true;
      viewer.disableObjectLayer(line, s3dLayerType.viewLayer, true);
      viewer.enableObjectLayer(line, s3dLayerType.editLayer, true);
  ...
  this.addAxisTexts = function (lineInfos, direction, textParameter) {
    ...
    for (let i = 0; i < lineInfos.length; i++) {
      ...
      //显示值			 
      let textGeo = new TextGeometry(cmnPcr.decimalToStr(common3DFunction.s2v(lineInfo.value, thatS3dAxis.manager.viewer.distanceRatio), false, 0), textParameter);
      let textMesh = new Mesh(textGeo, thatS3dAxis.textMaterial);
      let textBox = new Box3().setFromObject(textMesh);
      textMesh.position.set(-(textBox.max.x + textBox.min.x) / 2, -(textBox.max.y + textBox.min.y) / 2, -(textBox.max.z + textBox.min.z) / 2);
      let textObject3D = new Object3D();
      let position = null;
      switch (direction) {
        case "left":
          {
      ...
      viewer.disableObjectLayer(textObject3D, s3dLayerType.viewLayer, true);
      viewer.enableObjectLayer(textObject3D, s3dLayerType.editLayer, true);
      viewer.scene.add(textObject3D);
    }
  };
};

export { S3dAxis as default };
