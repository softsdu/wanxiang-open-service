import { MeshStandardMaterial, FrontSide, LineBasicMaterial, DoubleSide, MeshBasicMaterial, LineDashedMaterial, BoxGeometry, Mesh, BufferGeometry, Line, PlaneGeometry, Group, Vector3, EllipseCurve, Object3D, Box3 } from '../../node_modules/three/build/three.module.js';
import './s3dPathDrawingTool.css.js';
import { FontLoader } from '../../node_modules/three/examples/jsm/loaders/FontLoader.js';
import { TextGeometry } from '../../node_modules/three/examples/jsm/geometries/TextGeometry.js';
import { msgBox, s3dUiStatus, cmnPcr } from '../../commonjs/common/common.js';
...
let S3dPathDrawingTool = function () {
  ...
  this.fontConfig = {
    font: null,
    size: this.constraintMarkSize * 3 / 4,
    // 文本大小
    height: 0.01 // 文本厚度
  };
  this.selectedPointMaterial = new MeshStandardMaterial({
    color: 0xFF0000,
    depthTest: false,
    flatShading: false,
    side: FrontSide,
  ...
  this.pointMaterial = new MeshStandardMaterial({
    color: 0x00FF00,
    depthTest: false,
    flatShading: false,
    side: FrontSide,
  ...
  this.selectedLineMaterial = new LineBasicMaterial({
    color: 0xFF0000
  });
  this.lineMaterial = new LineBasicMaterial({
    color: 0xEEEEEE
  });
  this.planeMaterial = new LineBasicMaterial({
    color: 0x888888,
    side: DoubleSide,
    opacity: 0.75,
    transparent: true
  });
  this.constraintTextMaterial = new MeshBasicMaterial({
    color: 0xFF00FF
  });
  this.constraintLineMaterial = new LineBasicMaterial({
    color: 0xFF00FF
  });
  this.tagTextMaterial = new MeshBasicMaterial({
    color: 0xFFE97F
  });
  this.tagLineMaterial = new LineDashedMaterial({
    color: 0xFFE97F,
  ...
  this.addConstraintDistance = function () {
    ...
    if (lineNames.length !== 1) {
      ...
      if (existConstraintName != null) {
        msgBox.alert({
          info: "已定义距离约束: " + existConstraintName
        });
      } else {
  ...
  this.addConstraintParallel = function () {
    ...
    if (lineNames.length !== 2) {
      ...
      if (existConstraintName != null) {
        ...
        thatS3dPathDrawingTool.movePointByName(affectedPointInfo.name, common3DFunction.s2v(affectedPointInfo.x, thatS3dPathDrawingTool.manager.viewer.distanceRatio), common3DFunction.s2v(affectedPointInfo.y, thatS3dPathDrawingTool.manager.viewer.distanceRatio));
      }
    }
  };
  ...
  this.getAngleByThreePoints = function (fromPoint, toPoint, centerPoint) {
    let endAngle = common2DLine.getAngle({
      ...
      x: fromPoint.x - centerPoint.x,
      y: fromPoint.y - centerPoint.y
    });
    let startAngle = common2DLine.getAngle({
      x: 1,
      y: 0
    }, {
      x: toPoint.x - centerPoint.x,
      y: toPoint.y - centerPoint.y
    });
  ...
  this.addCreatingFromObject = function (fromPosition) {
    let pointGeometry = new BoxGeometry(thatS3dPathDrawingTool.pointSize, thatS3dPathDrawingTool.pointSize, thatS3dPathDrawingTool.pointSize);
    let pointObject3D = new Mesh(pointGeometry, thatS3dPathDrawingTool.selectedPointMaterial);
    pointObject3D.position.set(fromPosition.x, thatS3dPathDrawingTool.objectZero, fromPosition.y);
  ...
  this.addCreatingToObject = function (toPosition) {
    let pointGeometry = new BoxGeometry(thatS3dPathDrawingTool.pointSize, thatS3dPathDrawingTool.pointSize, thatS3dPathDrawingTool.pointSize);
    let pointObject3D = new Mesh(pointGeometry, thatS3dPathDrawingTool.selectedPointMaterial);
    pointObject3D.position.set(toPosition.x, thatS3dPathDrawingTool.objectZero, toPosition.y);
    pointObject3D.isCreatingObject = true;
    thatS3dPathDrawingTool.rootObject3D.add(pointObject3D);
      ...
      case "segment":
        {
          ...
          lineObject3D = new Line(geometry, thatS3dPathDrawingTool.selectedLineMaterial);
          break;
        }
      case "arc":
        {
          ...
          let geometry = new BufferGeometry().setFromPoints(point3Ds);
          lineObject3D = new Line(geometry, thatS3dPathDrawingTool.lineMaterial);
          break;
        }
      default:
        {
  ...
  this.changeConstraintValue = function (name, value) {
    ...
    switch (constraintInfo.constraintType) {
      case "distance":
        {
          let distance = common3DFunction.v2s(value, thatS3dPathDrawingTool.manager.viewer.distanceRatio);
  ...
  this.calcAffectedPointInfoByConstraintAngle = function (fromLineName, toLineName, angle) {
    ...
    if (anglePointNames.center == null) {
      ...
      if (fromLineLength === 0) {
        ...
        let toLenPoint = common2DLine.getPointInLine({
          x: 0,
          y: 0
        }, toNormalPoint, toLineLength);
        return {
          name: angleToPointInfo.name,
  ...
  this.removePathObjects = function (names, withoutConfirm) {
    if (!withoutConfirm && !msgBox.confirm({
      info: "确定删除吗?"
    })) {
      return;
    }
    ...
    for (let lineName in allNeedRemoveLineNameMap) {
      ...
      lineObject3D.userData.info.lineInfo;
  ...
  this.getRelatedLineNames = function (pointObject3D, allLineObjectMap) {
    ...
    for (let lineName in allLineObjectMap) {
      ...
      switch (lineObject.userData.info.lineInfo.lineType) {
        case "segment":
          {
            if (lineObject.userData.info.lineInfo.from === pointName || lineObject.userData.info.lineInfo.to === pointName) {
        ...
        case "arc":
          {
            if (lineObject.userData.info.lineInfo.from === pointName || lineObject.userData.info.lineInfo.to === pointName || lineObject.userData.info.lineInfo.center === pointName) {
              lineNames.push(lineName);
            }
            break;
          }
  ...
  this.getSegmentNameByPointNames = function (fromPointName, toPointName) {
    ...
    for (let name in allLineObjectMap) {
      let lineObject = allLineObjectMap[name];
      let lineInfo = lineObject.userData.info.lineInfo;
      if (lineInfo.lineType === "segment" && (lineInfo.from === fromPointName && lineInfo.to === toPointName || lineInfo.from === toPointName && lineInfo.to === fromPointName)) {
        return name;
      }
    }
    return null;
  ...
  this.getArcNameByPointNames = function (fromPointName, toPointName, centerPointName) {
    ...
    for (let name in allLineObjectMap) {
      let lineObject = allLineObjectMap[name];
      let lineInfo = lineObject.userData.info.lineInfo;
      if (lineInfo.lineType === "arc" && lineInfo.from === fromPointName && lineInfo.center === centerPointName && lineInfo.to === toPointName) {
        return name;
      }
    }
    return null;
  ...
  this.getToolbarHtml = function () {
    let html = "<div class=\"pathDrawingToolContainer pathDrawingToolToolbarContainer\">" + "<div class=\"pathDrawingToolBackground\"></div>" + "<div class=\"pathDrawingToolHeader\"><div class=\"pathDrawingToolTitle\">工具箱</div></div>" + "<div class=\"pathDrawingToolInnerContainer pathDrawingToolInnerPathContainer\">" + "<div class=\"pathDrawingToolToolbarBtnContainer pathDrawingToolToolbarBtnPoint\" name=\"point\" title=\"点\">" + "<svg xmlns=\"http://www.w3.org/2000/svg\" class=\"pathDrawingToolToolbarBtnSvg\" width=\"32\" height=\"32\"><rect x=\"12\" y=\"12\" width=\"6\" height=\"6\" fill=\"#00FF00\" shape-rendering=\"crispEdges\"/></svg>" + "</div>" + "<div class=\"pathDrawingToolToolbarBtnContainer pathDrawingToolToolbarBtnSegment\" name=\"segment\" title=\"线\">" + "<svg xmlns=\"http://www.w3.org/2000/svg\" class=\"pathDrawingToolToolbarBtnSvg\" width=\"32\" height=\"32\"><rect x=\"5\" y=\"5\" width=\"6\" height=\"6\" fill=\"#00FF00\" shape-rendering=\"crispEdges\"/><rect x=\"19\" y=\"19\" width=\"6\" height=\"6\" fill=\"#00FF00\" shape-rendering=\"crispEdges\"/><line x1=\"8\" y1=\"8\" x2=\"22\" y2=\"22\" stroke=\"#FFFFFF\" stroke-width=\"1\" shape-rendering=\"crispEdges\"/></svg>" + "</div>" + "<div class=\"pathDrawingToolToolbarBtnContainer pathDrawingToolToolbarBtnArc\" name=\"arc\" title=\"弧\">" + "<svg xmlns=\"http://www.w3.org/2000/svg\" class=\"pathDrawingToolToolbarBtnSvg\" width=\"32\" height=\"32\"><rect x=\"5\" y=\"5\" width=\"6\" height=\"6\" fill=\"#00FF00\" shape-rendering=\"crispEdges\"/><rect x=\"19\" y=\"19\" width=\"6\" height=\"6\" fill=\"#00FF00\" shape-rendering=\"crispEdges\"/><path d=\"M8,8 A14,14 0 0 1 22,22\" stroke=\"#FFFFFF\" stroke-width=\"1\" fill=\"none\" shape-rendering=\"crispEdges\"/></svg>" + "</div>" + "</div>" + "<div class=\"pathDrawingToolInnerContainer pathDrawingToolInnerConstraintContainer\">" + "<div class=\"pathDrawingToolToolbarBtnContainer pathDrawingToolToolbarBtnPoint\" name=\"horizontal\" title=\"水平约束\">" + "<svg xmlns=\"http://www.w3.org/2000/svg\" class=\"pathDrawingToolToolbarBtnSvg\" width=\"32\" height=\"32\"><text x=\"15\" y=\"18\" class=\"toolbarText\" stroke=\"#FF00FF\" text-anchor=\"middle\" dominant-baseline=\"central\" shape-rendering=\"crispEdges\">H</text><line x1=\"5\" y1=\"10\" x2=\"25\" y2=\"10\" stroke=\"#FFFFFF\" stroke-width=\"1\" shape-rendering=\"crispEdges\"/></svg>" + "</div>" + "<div class=\"pathDrawingToolToolbarBtnContainer pathDrawingToolToolbarBtnPoint\" name=\"vertical\" title=\"竖直约束\">" + "<svg xmlns=\"http://www.w3.org/2000/svg\" class=\"pathDrawingToolToolbarBtnSvg\" width=\"32\" height=\"32\"><text x=\"12\" y=\"15\" class=\"toolbarText\" stroke=\"#FF00FF\" text-anchor=\"middle\" dominant-baseline=\"central\" shape-rendering=\"crispEdges\">V</text><line x1=\"20\" y1=\"5\" x2=\"20\" y2=\"25\" stroke=\"#FFFFFF\" stroke-width=\"1\" shape-rendering=\"crispEdges\"/></svg>" + "</div>" + "<div class=\"pathDrawingToolToolbarBtnContainer pathDrawingToolToolbarBtnPoint\" name=\"distance\" title=\"距离约束\">" + "<svg xmlns=\"http://www.w3.org/2000/svg\" class=\"pathDrawingToolToolbarBtnSvg\" width=\"32\" height=\"32\"><text x=\"16\" y=\"12\" class=\"toolbarText\" stroke=\"#FF00FF\" text-anchor=\"middle\" dominant-baseline=\"central\" shape-rendering=\"crispEdges\">50</text><line x1=\"5\" y1=\"20\" x2=\"25\" y2=\"20\" stroke=\"#FFFFFF\" stroke-width=\"1\" shape-rendering=\"crispEdges\"/></svg>" + "</div>" + "<div class=\"pathDrawingToolToolbarBtnContainer pathDrawingToolToolbarBtnPoint\" name=\"parallel\" title=\"平行约束\">" + "<svg xmlns=\"http://www.w3.org/2000/svg\" class=\"pathDrawingToolToolbarBtnSvg\" width=\"32\" height=\"32\"><line x1=\"4\" y1=\"12\" x2=\"19\" y2=\"27\" stroke=\"#FFFFFF\" stroke-width=\"1\" shape-rendering=\"crispEdges\"/><line x1=\"12\" y1=\"4\" x2=\"27\" y2=\"19\" stroke=\"#FFFFFF\" stroke-width=\"1\" shape-rendering=\"crispEdges\"/></svg>" + "</div>" + "<div class=\"pathDrawingToolToolbarBtnContainer pathDrawingToolToolbarBtnPoint\" name=\"angle\" title=\"角度约束\">" + "<svg xmlns=\"http://www.w3.org/2000/svg\" class=\"pathDrawingToolToolbarBtnSvg\" width=\"32\" height=\"32\"><text x=\"20\" y=\"17\" class=\"toolbarText\" stroke=\"#FF00FF\" text-anchor=\"middle\" dominant-baseline=\"central\" shape-rendering=\"crispEdges\">60°</text><line x1=\"5\" y1=\"25\" x2=\"25\" y2=\"25\" stroke=\"#FFFFFF\" stroke-width=\"1\" shape-rendering=\"crispEdges\"/><line x1=\"5\" y1=\"25\" x2=\"15\" y2=\"5\" stroke=\"#FFFFFF\" stroke-width=\"1\" shape-rendering=\"crispEdges\"/></svg>" + "</div>" + "</div>" + "</div>";
    return html;
  };
  this.getOperationHtml = function () {
    let html = "<div class=\"pathDrawingToolContainer pathDrawingToolOperationContainer\">" + "<div class=\"pathDrawingToolBackground\"></div>" + "<div class=\"pathDrawingToolHeader\"><div class=\"pathDrawingToolTitle\">操作</div></div>" + "<div class=\"pathDrawingToolInnerContainer pathDrawingToolInnerOperationContainer\">" + "<div class=\"pathDrawingToolOperationBtnContainer\" name=\"cancel\" title=\"取消编辑\">取消</div>" + "<div class=\"pathDrawingToolOperationBtnContainer\" name=\"ok\" title=\"确定返回结果\">确定</div>" + "</div>" + "</div>";
    return html;
  };
  ...
  this.sourceToPathInfo = function (sourceInfo) {
    ...
    if (sourceInfo.pointStr != null) {
      ...
      for (let i = 0; i < pStrs.length; i++) {
        ...
        let strs = pStr.split(",");
        let name = strs[0];
        let x = common3DFunction.v2s(parseFloat(strs[1]), thatS3dPathDrawingTool.manager.viewer.distanceRatio);
        let y = common3DFunction.v2s(parseFloat(strs[2]), thatS3dPathDrawingTool.manager.viewer.distanceRatio);
        pointMap[name] = {
          name: name,
          x: x,
          y: y
    ...
    if (sourceInfo.constraintStr != null) {
      ...
      for (let i = 0; i < constraintStrs.length; i++) {
        ...
        switch (constraintType) {
          case "distance":
            {
              value = common3DFunction.v2s(parseFloat(strs[4]), thatS3dPathDrawingTool.manager.viewer.distanceRatio);
              break;
            }
          case "angle":
            {
  ...
  this.initDrawingPlane = function () {
    ...
    let planeGeometry = new PlaneGeometry(sizeX * planeScale, sizeZ * planeScale);
    let plane = new Mesh(planeGeometry, thatS3dPathDrawingTool.planeMaterial);
  ...
  this.createPointObject3D = function (pointInfo) {
    let pointGeometry = new BoxGeometry(thatS3dPathDrawingTool.pointSize, thatS3dPathDrawingTool.pointSize, thatS3dPathDrawingTool.pointSize);
    let pointObject3D = new Mesh(pointGeometry, thatS3dPathDrawingTool.pointMaterial);
    pointObject3D.position.set(pointInfo.x, thatS3dPathDrawingTool.objectZero, pointInfo.y);
  ...
  this.calcSegmentPoints = function (fromPointInfo, toPointInfo) {
    let points = [];
    points.push(new Vector3(fromPointInfo.x, thatS3dPathDrawingTool.objectZero, fromPointInfo.y));
    points.push(new Vector3(toPointInfo.x, thatS3dPathDrawingTool.objectZero, toPointInfo.y));
    return points;
  };
  this.createSegmentObject3D = function (lineInfo, pathInfo) {
    ...
    let geometry = new BufferGeometry().setFromPoints(points);
    let object3D = new Line(geometry, thatS3dPathDrawingTool.lineMaterial);
  ...
  this.calcArcPoint3Ds = function (fromPointInfo, toPointInfo, centerPointInfo) {
    if (fromPointInfo == null || toPointInfo == null) {
      return [{
        x: centerPointInfo.x,
        y: thatS3dPathDrawingTool.objectZero,
        z: centerPointInfo.y
      }];
    } else {
      //因为坐标系问题，开始和结束互换
      let endAngle = common2DLine.getAngle({
        ...
        x: fromPointInfo.x - centerPointInfo.x,
        y: fromPointInfo.y - centerPointInfo.y
      });
      let startAngle = common2DLine.getAngle({
        x: 1,
        y: 0
      }, {
        x: toPointInfo.x - centerPointInfo.x,
        y: toPointInfo.y - centerPointInfo.y
      });
      if (startAngle > endAngle) {
        endAngle += 2 * Math.PI;
      }
      let radius = common2DLine.getLength(fromPointInfo, centerPointInfo);
      let arcCurve = new EllipseCurve(centerPointInfo.x, centerPointInfo.y, radius, radius, startAngle, endAngle, true, 0);
      let points = arcCurve.getPoints(100);
      let point3Ds = [];
      for (let i = 0; i < points.length; i++) {
        let point = points[i];
        let point3D = new Vector3(point.x, thatS3dPathDrawingTool.objectZero, point.y);
  ...
  this.calcArcRadiusPoints = function (centerPointInfo, arcCenterPoint) {
    let points = [];
    points.push(new Vector3(centerPointInfo.x, thatS3dPathDrawingTool.objectZero, centerPointInfo.y));
    points.push(new Vector3(arcCenterPoint.x, thatS3dPathDrawingTool.objectZero, arcCenterPoint.y));
    return points;
  };
  ...
  this.createArcObject3D = function (lineInfo, pathInfo) {
    ...
    let arcGeometry = new BufferGeometry().setFromPoints(arcPoint3Ds);
    let object3D = new Line(arcGeometry, thatS3dPathDrawingTool.lineMaterial);
    object3D.userData.info = {
      type: "line",
      name: lineInfo.name,
      lineInfo: lineInfo
  ...
  this.createArcTagObject3D = function (lineInfo, pathInfo) {
    ...
    let radiusLineGeometry = new BufferGeometry().setFromPoints(radiusLinePoints);
    let radiusLineObject3D = new Line(radiusLineGeometry, thatS3dPathDrawingTool.tagLineMaterial);
    ...
    let radiusText = "R:" + cmnPcr.decimalToStr(common3DFunction.s2v(radiusValue, thatS3dPathDrawingTool.manager.viewer.distanceRatio), false, thatS3dPathDrawingTool.positionFixNum);
    let textGeometry = new TextGeometry(radiusText, thatS3dPathDrawingTool.fontConfig);
    let textObject3D = new Mesh(textGeometry, thatS3dPathDrawingTool.tagTextMaterial);
    textObject3D.rotation.set(-Math.PI / 2, 0, 0);
    let radiusTextObject3D = new Object3D();
    radiusTextObject3D.add(textObject3D);
    radiusTextObject3D.position.set(textPosition.x, thatS3dPathDrawingTool.objectZero, textPosition.y);
  ...
  this.createConstraintDistanceObject3D = function (constraintInfo, pathInfo) {
    let text = cmnPcr.decimalToStr(common3DFunction.s2v(constraintInfo.value, thatS3dPathDrawingTool.manager.viewer.distanceRatio), false, thatS3dPathDrawingTool.positionFixNum);
    let textGeometry = new TextGeometry(text, thatS3dPathDrawingTool.fontConfig);
    let textObject3D = new Mesh(textGeometry, thatS3dPathDrawingTool.constraintTextMaterial);
    textObject3D.rotation.set(-Math.PI / 2, 0, 0);
    let textBox = new Box3().setFromObject(textObject3D);
    ...
    object3D.position.set((fromPosition.x + toPosition.x) / 2, thatS3dPathDrawingTool.objectZero, (fromPosition.y + toPosition.y) / 2);
  ...
  this.createConstraintAngleObject3D = function (constraintInfo, pathInfo) {
    ...
    let angleArcGeometry = new BufferGeometry().setFromPoints(angleArcPoint3Ds);
    let angleArcObject3D = new Line(angleArcGeometry, thatS3dPathDrawingTool.constraintLineMaterial);
    ...
    let textBox = new Box3().setFromObject(textObject3D);
    textObject3D.position.set(-(textBox.max.x + textBox.min.x) / 2, -(textBox.max.y + textBox.min.y) / 2, -(textBox.max.z + textBox.min.z) / 2);
    let angleTextObject3D = new Object3D();
    ...
    angleTextObject3D.position.set(angleTextPosition.x, thatS3dPathDrawingTool.objectZero, angleTextPosition.y);
    let object3D = new Object3D();
  ...
  this.createConstraintParallelObject3D = function (constraintInfo, pathInfo) {
    ...
    let parallelFromObject3D = new Object3D();
    ...
    parallelFromObject3D.position.set((fromTopPosition.x + toTopPosition.x) / 2, thatS3dPathDrawingTool.objectZero, (fromTopPosition.y + toTopPosition.y) / 2);
    let yFromRotate = -common2DLine.getAngle({
      x: 1,
      y: 0
    }, {
      x: fromTopPosition.x - toTopPosition.x,
      y: fromTopPosition.y - toTopPosition.y
    });
    ...
    parallelToObject3D.position.set((fromBottomPosition.x + toBottomPosition.x) / 2, thatS3dPathDrawingTool.objectZero, (fromBottomPosition.y + toBottomPosition.y) / 2);
    let yToRotate = -common2DLine.getAngle({
      x: 1,
      y: 0
    }, {
      x: fromBottomPosition.x - toBottomPosition.x,
      y: fromBottomPosition.y - toBottomPosition.y
    });
  ...
  this.refreshPointTable = function () {
    ...
    for (let name in pathInfo.pointMap) {
      ...
      $(item).find(".pathDrawingCellInputNumber[name='x']").val(cmnPcr.decimalToStr(common3DFunction.s2v(pointInfo.x, thatS3dPathDrawingTool.manager.viewer.distanceRatio), false, thatS3dPathDrawingTool.positionFixNum));
      $(item).find(".pathDrawingCellInputNumber[name='y']").val(cmnPcr.decimalToStr(common3DFunction.s2v(pointInfo.y, thatS3dPathDrawingTool.manager.viewer.distanceRatio), false, thatS3dPathDrawingTool.positionFixNum));
    }
  };
  ...
  this.getAllTableHtml = function (pathInfo) {
    let html = "<div class=\"pathDrawingToolContainer pathDrawingToolPointContainer\" name=\"point\">" + "<div class=\"pathDrawingToolBackground\"></div>" + "<div class=\"pathDrawingToolHeader\"><div class=\"pathDrawingToolTitle\">点&nbsp;<span class=\"pathDrawingItemCount\"></span></div></div>" + "<div class=\"pathDrawingToolInnerContainer\">" + thatS3dPathDrawingTool.getAllPointHtml(pathInfo) + "</div>" + "</div>" + "<div class=\"pathDrawingToolContainer pathDrawingToolLineContainer\" name=\"line\">" + "<div class=\"pathDrawingToolBackground\"></div>" + "<div class=\"pathDrawingToolHeader\"><div class=\"pathDrawingToolTitle\">线&nbsp;<span class=\"pathDrawingItemCount\"></span></div></div>" + "<div class=\"pathDrawingToolInnerContainer\">" + thatS3dPathDrawingTool.getAllLineHtml(pathInfo) + "</div>" + "</div>" + "<div class=\"pathDrawingToolContainer pathDrawingToolConstraintContainer\" name=\"constraint\">" + "<div class=\"pathDrawingToolBackground\"></div>" + "<div class=\"pathDrawingToolHeader\"><div class=\"pathDrawingToolTitle\">约束&nbsp;<span class=\"pathDrawingItemCount\"></span></div></div>" + "<div class=\"pathDrawingToolInnerContainer\">" + thatS3dPathDrawingTool.getAllConstraintHtml(pathInfo) + "</div>" + "</div>";
    return html;
  };
  this.getAllPointHtml = function (pathInfo) {
    let html = "<div class=\"pathDrawingTableHeader\">" + "<div class=\"pathDrawingCell pathDrawingCellName\">名称</div>" + "<div class=\"pathDrawingCell pathDrawingCellString\" style=\"width:74px\">横坐标</div>" + "<div class=\"pathDrawingCell pathDrawingCellString\" style=\"width:74px\">纵坐标</div>" + "<div class=\"pathDrawingCell pathDrawingCellBtnHeader\">操作</div>" + "</div>";
    for (let name in pathInfo.pointMap) {
      let pointInfo = pathInfo.pointMap[name];
  ...
  this.getPointHtml = function (pointInfo) {
    let xValue = cmnPcr.toFixed(common3DFunction.s2v(pointInfo.x, thatS3dPathDrawingTool.manager.viewer.distanceRatio), thatS3dPathDrawingTool.positionFixNum);
    let yValue = cmnPcr.toFixed(common3DFunction.s2v(pointInfo.y, thatS3dPathDrawingTool.manager.viewer.distanceRatio), thatS3dPathDrawingTool.positionFixNum);
    return "<div class=\"pathDrawingTableRow\" name=\"" + pointInfo.name + "\">" + "<div class=\"pathDrawingCell pathDrawingCellName\">" + pointInfo.name + "</div>" + "<input class=\"pathDrawingCell pathDrawingCellInputNumber\" type=\"number\" name=\"x\" value=\"" + xValue + "\"/>" + "<input class=\"pathDrawingCell pathDrawingCellInputNumber\" type=\"number\" name=\"y\" value=\"" + yValue + "\"/>" + "<div class=\"pathDrawingCell pathDrawingCellBtn\" name=\"remove\">×</div>" + "</div>";
  };
  this.getAllLineHtml = function (pathInfo) {
    let html = "<div class=\"pathDrawingTableHeader\">" + "<div class=\"pathDrawingCell pathDrawingCellName\">名称</div>" + "<div class=\"pathDrawingCell pathDrawingCellString\">类型</div>" + "<div class=\"pathDrawingCell pathDrawingCellString\">点A</div>" + "<div class=\"pathDrawingCell pathDrawingCellString\">点B</div>" + "<div class=\"pathDrawingCell pathDrawingCellString\">点C</div>" + "<div class=\"pathDrawingCell pathDrawingCellBtnHeader\">操作</div>" + "</div>";
    for (let name in pathInfo.lineMap) {
  ...
  this.getLineHtml = function (lineInfo) {
    return "<div class=\"pathDrawingTableRow\" name=\"" + lineInfo.name + "\">" + "<div class=\"pathDrawingCell pathDrawingCellName\">" + lineInfo.name + "</div>" + "<div class=\"pathDrawingCell pathDrawingCellString\" name=\"lineType\">" + thatS3dPathDrawingTool.getLineTypeText(lineInfo.lineType) + "</div>" + "<div class=\"pathDrawingCell pathDrawingCellString\" name=\"from\">" + lineInfo.from + "</div>" + "<div class=\"pathDrawingCell pathDrawingCellString\" name=\"to\">" + lineInfo.to + "</div>" + "<div class=\"pathDrawingCell pathDrawingCellString\" name=\"center\">" + (lineInfo.center == null ? "-" : lineInfo.center) + "</div>" + "<div class=\"pathDrawingCell pathDrawingCellBtn\" name=\"remove\">×</div>" + "</div>";
  };
  ...
  this.getAllConstraintHtml = function (pathInfo) {
    let html = "<div class=\"pathDrawingTableHeader\">" + "<div class=\"pathDrawingCell pathDrawingCellName\">名称</div>" + "<div class=\"pathDrawingCell pathDrawingCellString\">类型</div>" + "<div class=\"pathDrawingCell pathDrawingCellString\">相关A</div>" + "<div class=\"pathDrawingCell pathDrawingCellString\">相关B</div>" + "<div class=\"pathDrawingCell pathDrawingCellString\" style=\"width:60px\">值</div>" + "<div class=\"pathDrawingCell pathDrawingCellBtnHeader\">操作</div>" + "</div>";
    for (let name in pathInfo.constraintMap) {
  ...
  this.getConstraintHtml = function (constraintInfo) {
    ...
    switch (constraintInfo.constraintType) {
      case "distance":
        {
          value = cmnPcr.decimalToStr(common3DFunction.s2v(constraintInfo.value, thatS3dPathDrawingTool.manager.viewer.distanceRatio), false, thatS3dPathDrawingTool.positionFixNum);
          break;
        }
      case "angle":
        {
    ...
    let html = "<div class=\"pathDrawingTableRow\" name=\"" + constraintInfo.name + "\">" + "<div class=\"pathDrawingCell pathDrawingCellName\">" + constraintInfo.name + "<span class=\"pathDrawingCellNoneError\">?</span></div>" + "<div class=\"pathDrawingCell pathDrawingCellString\" name=\"constraintType\">" + thatS3dPathDrawingTool.getConstraintTypeText(constraintInfo.constraintType) + "</div>" + "<div class=\"pathDrawingCell pathDrawingCellString\" name=\"from\">" + constraintInfo.from + "</div>" + "<div class=\"pathDrawingCell pathDrawingCellString\" name=\"to\">" + (constraintInfo.to == null ? "-" : constraintInfo.to) + "</div>";
    switch (constraintInfo.constraintType) {
      case "distance":
        {
    ...
    html += "<div class=\"pathDrawingCell pathDrawingCellBtn\" name=\"remove\">×</div>" + "</div>";
    return html;
  };
  ...
  this.getPathObject = function (checkObj) {
    while (checkObj.type !== "Scene") {
      if (checkObj.parent.isPathRootObject) {
        if (checkObj.visible && checkObj.userData.info != null && (checkObj.userData.info.type === "line" || checkObj.userData.info.type === "point")) {
          return checkObj;
        } else {
          return null;
        }
  ...
  this.selectPathObject3D = function (object3D) {
    $("#" + thatS3dPathDrawingTool.containerId);
  ...
  this.moveCreatingPoint = function (position3D) {
    thatS3dPathDrawingTool.creatingInfo.fromObject.position.set(position3D.x, thatS3dPathDrawingTool.objectZero, position3D.z);
  };
  ...
  this.moveCreatingSegmentToPoint = function (position3D) {
    thatS3dPathDrawingTool.creatingInfo.toObject.position.set(position3D.x, thatS3dPathDrawingTool.objectZero, position3D.z);
    ...
    thatS3dPathDrawingTool.creatingInfo.lineObject.geometry = new BufferGeometry().setFromPoints(points);
  };
  ...
  this.movePointByName = function (pointName, newXmm, newYmm, affectedPointNameMap) {
    ...
    let shiftX = common3DFunction.v2s(newXmm, thatS3dPathDrawingTool.manager.viewer.distanceRatio) - object3D.position.x;
    let shiftY = common3DFunction.v2s(newYmm, thatS3dPathDrawingTool.manager.viewer.distanceRatio) - object3D.position.z;
  ...
  this.movePath = function (selectedNameMap, shiftX, shiftZ, absoluteX, absoluteZ, affectedPointNameMap) {
          ...
          case "arc":
            {
              ...
              if (allNeedMovePointMap[lineObject.userData.info.lineInfo.from] && allNeedMovePointMap[lineObject.userData.info.lineInfo.to] || allNeedMovePointMap[lineObject.userData.info.lineInfo.center] && allNeedMovePointMap[lineObject.userData.info.lineInfo.from] || allNeedMovePointMap[lineObject.userData.info.lineInfo.center] && allNeedMovePointMap[lineObject.userData.info.lineInfo.to]) {
    ...
    if (needMovePointCount > 0) {
      ...
      for (let name in allLineObjectMap) {
        let lineObject = allLineObjectMap[name];
        if (lineObject.userData.info.lineInfo.lineType === "arc" && !(allNeedMovePointMap[lineObject.userData.info.lineInfo.center] && allNeedMovePointMap[lineObject.userData.info.lineInfo.from] && allNeedMovePointMap[lineObject.userData.info.lineInfo.to])) {
          let fromObject = allPointObjectMap[lineObject.userData.info.lineInfo.from];
          let toObject = allPointObjectMap[lineObject.userData.info.lineInfo.to];
      ...
      for (let name in allLineObjectMap) {
        ...
        switch (lineObject.userData.info.lineInfo.lineType) {
          case "arc":
            {
              if (allNeedMovePointMap[lineObject.userData.info.lineInfo.center] || allNeedMovePointMap[lineObject.userData.info.lineInfo.from] || allNeedMovePointMap[lineObject.userData.info.lineInfo.to]) {
                let fromObject = allPointObjectMap[lineObject.userData.info.lineInfo.from];
                let toObject = allPointObjectMap[lineObject.userData.info.lineInfo.to];
                let centerObject = allPointObjectMap[lineObject.userData.info.lineInfo.center];
                let centerPoint = {
                ...
                lineObject.geometry = new BufferGeometry().setFromPoints(points);
  ...
  this.reCalcPointsByConstraints = function (pathInfo, allPointObjectMap, selectedNameMap, affectedPointNameMap) {
    ...
    for (let pointName in newAffectPointNameMap) {
      let pointInfo = pathInfo.pointMap[pointName];
      thatS3dPathDrawingTool.movePointByName(pointInfo.name, common3DFunction.s2v(pointInfo.x, thatS3dPathDrawingTool.manager.viewer.distanceRatio), common3DFunction.s2v(pointInfo.y, thatS3dPathDrawingTool.manager.viewer.distanceRatio), affectedPointNameMap);
    }
  };
  this.reLocatePointAndInfo = function (pointName, x, y, allPointObjectMap, pathInfo) {
    ...
    pointInfo.y = y;
    let pointObject = allPointObjectMap[pointName];
    pointObject.position.set(x, thatS3dPathDrawingTool.objectZero, y);
  };
  ...
  this.checkConstraintEffectiveParallel = function (constraintInfo, pathInfo) {
    ...
    let toLineFromPointInfo = pathInfo.pointMap[toLineInfo.from];
    let toLineToPointInfo = pathInfo.pointMap[toLineInfo.to];
    return common2DLine.checkParallel(fromLineFromPointInfo, fromLineToPointInfo, toLineFromPointInfo, toLineToPointInfo, thatS3dPathDrawingTool.ignoreAngle) || common2DLine.checkParallel(fromLineFromPointInfo, fromLineToPointInfo, toLineToPointInfo, toLineFromPointInfo, thatS3dPathDrawingTool.ignoreAngle);
  };
  this.checkConstraintEffectiveAngle = function (constraintInfo, pathInfo) {
    let anglePointInfo = thatS3dPathDrawingTool.getAnglePointsByTwoLines(constraintInfo.from, constraintInfo.to, pathInfo);
    if (anglePointInfo == null || anglePointInfo.from == null || anglePointInfo.to == null || anglePointInfo.center == null) {
      return false;
    } else {
      let angle = thatS3dPathDrawingTool.getAngleByThreePoints(anglePointInfo.from, anglePointInfo.to, anglePointInfo.center);
  ...
  this.checkIsLoop = function (pathInfo) {
    ...
    if (lastLineName != null) {
      ...
      if (!(lastLineInfo.from === firstPointName && lastLineInfo.to === lastPointName) && !(lastLineInfo.from === lastPointName && lastLineInfo.to === firstPointName)) return false;
    }
  ...
  this.pointMapToStr = function (pathInfo) {
    ...
    for (let pointName in pathInfo.pointMap) {
      let pointInfo = pathInfo.pointMap[pointName];
      let str = pointName + "," + cmnPcr.decimalToStr(common3DFunction.s2v(pointInfo.x, thatS3dPathDrawingTool.manager.viewer.distanceRatio), false, thatS3dPathDrawingTool.positionFixNum) + "," + cmnPcr.decimalToStr(common3DFunction.s2v(pointInfo.y, thatS3dPathDrawingTool.manager.viewer.distanceRatio), false, thatS3dPathDrawingTool.positionFixNum);
      strs.push(str);
    }
  ...
  this.lineMapToStr = function (pathInfo) {
    ...
    for (let lineName in pathInfo.lineMap) {
      let lineInfo = pathInfo.lineMap[lineName];
      let str = lineName + "," + lineInfo.lineType + "," + lineInfo.from + "," + lineInfo.to + "," + (lineInfo.center == null ? "" : lineInfo.center);
      strs.push(str);
    }
    return cmnPcr.arrayToString(strs, ";");
  };
  this.constraintMapToStr = function (pathInfo) {
    ...
    for (let constraintName in pathInfo.constraintMap) {
      let constraintInfo = pathInfo.constraintMap[constraintName];
      let str = constraintName + "," + constraintInfo.constraintType + "," + constraintInfo.from + "," + (constraintInfo.to == null ? "" : constraintInfo.to) + "," + (constraintInfo.value == null ? "" : constraintInfo.constraintType === "distance" ? cmnPcr.decimalToStr(common3DFunction.s2v(constraintInfo.value, thatS3dPathDrawingTool.manager.viewer.distanceRatio), false, thatS3dPathDrawingTool.fixNum) : constraintInfo.constraintType === "angle" ? constraintInfo.value * Math.PI / 180 : "");
      strs.push(str);
    }
    return cmnPcr.arrayToString(strs, ";");
  };
  this.pathInfoToStr = function (pathInfo) {
    ...
    for (let i = 0; i < loopedPointInfos.length; i++) {
      ...
      let pointStr = cmnPcr.decimalToStr(common3DFunction.s2v(pointInfo.x, thatS3dPathDrawingTool.manager.viewer.distanceRatio), false, thatS3dPathDrawingTool.fixNum) + "," + cmnPcr.decimalToStr(common3DFunction.s2v(pointInfo.y, thatS3dPathDrawingTool.manager.viewer.distanceRatio), false, thatS3dPathDrawingTool.fixNum) + "," + (pointInfo.linePointType === "arc" ? "c" : "") + "," + (pointInfo.clockDirectionType === "anticlockwise" ? "a" : pointInfo.clockDirectionType === "clockwise" ? "c" : "");
...
export { S3dPathDrawingTool as default };
