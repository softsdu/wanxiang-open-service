import { FrontSide, LineBasicMaterial, Box3, ImageUtils, MeshPhongMaterial, Object3D, Vector3, BufferGeometry, Line, Float32BufferAttribute, Uint16BufferAttribute, Mesh, MeshStandardMaterial, Euler, Vector2 } from '../../node_modules/three/build/three.module.js';
import { cmnPcr, msgBox, s3dElement3DType } from '../../commonjs/common/common.js';
...
let S3dServerObjectCreator = function () {
  ...
  this.getRequestParameters = function (objectJson, componentInfo) {
    if (!componentInfo) {
      componentInfo = thatS3dServerObjectCreator.getComponentInfo(objectJson.code, objectJson.versionNum);
    }
    let parameters = {};
    for (let paramName in componentInfo.parameters) {
      ...
      parameters[paramName] = {
        name: paramName,
        value: objectParam == null ? null : objectParam.value,
        isGeo: comParam.isGeo
      };
    }
    return parameters;
  };
  this.getObjectCacheKey = function (objectJson, detailLevel, viewLevel, componentInfo) {
    let parameters = thatS3dServerObjectCreator.getRequestParameters(objectJson, componentInfo);
    return thatS3dServerObjectCreator.getCacheKey(objectJson.code, objectJson.versionNum, parameters, objectJson.useWorldPosition, detailLevel, viewLevel);
  };
  ...
  this.addCache = function (p) {
    thatS3dServerObjectCreator.manager.object3DCache.addRefComponentObject3D(p.cacheKey, p.object3D, p.geoJson, p.sameObjectId, p.objectSetting, p.box, p.helperObject3D);
  };
  this.createObject3Ds = function (objectJsons, afterCreateObject3D, countInfo) {
    ...
    for (let i = 0; i < objectJsons.length; i++) {
      ...
      let componentInfo = thatS3dServerObjectCreator.getComponentInfo(objectJson.code, objectJson.versionNum);
      if (componentInfo == null && !componentInfoKeys[objectJson.code + "_" + objectJson.versionNum]) {
        componentInfoKeys[objectJson.code + "_" + objectJson.versionNum] = true;
    ...
    thatS3dServerObjectCreator.creatingInfo.objectJsons = objectJsons;
    ...
    if (needServerGetComponentInfos.length === 0) {
      ...
      thatS3dServerObjectCreator.createServerObjects(objectJsons);
    } else {
      //需要先获取componentInfo
      thatS3dServerObjectCreator.getComponentJsons(needServerGetComponentInfos, function () {
        thatS3dServerObjectCreator.createServerObjects(thatS3dServerObjectCreator.creatingInfo.objectJsons);
      });
    }
  };
  this.createServerObjects = function () {
    let objectJsons = thatS3dServerObjectCreator.creatingInfo.objectJsons;
    let pArray = [];
    //先构造objectSetting
    ...
    for (let i = 0; i < objectJsons.length; i++) {
      ...
      let cacheKey = thatS3dServerObjectCreator.getCacheKey(objectJson.code, objectJson.versionNum, parameters, objectJson.useWorldPosition, thatS3dServerObjectCreator.manager.viewer.detailLevel, thatS3dServerObjectCreator.manager.viewer.viewLevel);
      let hasCache = thatS3dServerObjectCreator.checkHasCache(cacheKey);
      let p = {
        code: objectJson.code,
        versionNum: objectJson.versionNum,
        parentId: objectJson.parentId,
        parameters: objectJson.parameters,
        ...
        objectSetting: {
          ...
          useWorldPosition: objectJson.useWorldPosition,
          parameters: objectJson.parameters,
          isServer: true,
  ...
  this.createObject3DAfterLoadResource = function (cacheKey, resultJson) {
    ...
    let serverObjectSetting = resultJson.objectSetting;
    ...
    if (resourceJson == null) {
      ...
      object3D = thatS3dServerObjectCreator.jsonToResourceObject3D(geoJson, null, resultJson.objectSetting);
    }
    let box = new Box3().setFromObject(object3D, true);
    let helperObject3D = thatS3dServerObjectCreator.createHelperRootObject3D(geoJson, serverMaterialJsons);
    thatS3dServerObjectCreator.callbackCreateObject3D(cacheKey, object3D, geoJson, serverObjectSetting.id, serverObjectSetting, box, helperObject3D);
    delete thatS3dServerObjectCreator.creatingInfo.cacheKey2InfoMap[cacheKey];
  };
  this.callbackCreateObject3D = function (cacheKey, object3D, geoJson, objectId, serverObjectSetting, box, helperObject3D) {
    thatS3dServerObjectCreator.addCache({
      cacheKey: cacheKey,
      object3D: object3D,
      geoJson: geoJson,
      sameObjectId: objectId,
      objectSetting: serverObjectSetting,
      box: box,
      helperObject3D: helperObject3D
    });

    //有可能serverObjectSetting里的属性值与objectSetting的属性值不同（例如服务器端修改了属性值），那么需要更新 added by ls 20220623
    let requestParameters = {};
    let refComponentInfo = thatS3dServerObjectCreator.getComponentInfo(serverObjectSetting.code, serverObjectSetting.versionNum);
    for (let paramName in serverObjectSetting.parameters) {
      let objectParameter = serverObjectSetting.parameters[paramName];
      ...
      if (refComponentInfoParameter != null) {
        requestParameters[paramName] = {
          ...
          value: objectParameter.value === undefined ? null : objectParameter.value,
          isGeo: refComponentInfoParameter.isGeo
        };
      }
    }
    let newCacheKey = thatS3dServerObjectCreator.manager.object3DCache.getComponentObject3DKey(refComponentInfo.code, refComponentInfo.versionNum, requestParameters, serverObjectSetting.useWorldPosition, thatS3dServerObjectCreator.manager.viewer.detailLevel, thatS3dServerObjectCreator.manager.viewer.viewLevel);
    if (newCacheKey !== cacheKey) {
      thatS3dServerObjectCreator.manager.object3DCache.addRefComponentObject3D(newCacheKey, object3D, geoJson, objectId, serverObjectSetting, box, helperObject3D);
    }
  ...
  this.createObject3DByCloneCache = function (p) {
    ...
    newObject3D.userData.info = thatS3dServerObjectCreator.createNodeData(p.objectSetting, cacheInfo.objectSetting);
    if (newObject3D.userData.info.useWorldPosition) {
      newObject3D.rotation.set(0, 0, 0);
      newObject3D.position.set(0, 0, 0);
      newObject3D.scale.set(1, 1, 1);
    } else {
  ...
  this.setObject3DUvsImage = function (object3D, uvs, objectSetting) {
    if (uvs != null) {
      let meshUvCount = 0;
      ...
      if (meshUvCount > 0 || isForced) {
        for (let i = 0; i < object3D.children.length; i++) {
          ...
          thatS3dServerObjectCreator.setMeshUvsImage(mesh, meshUv, objectSetting);
        }
      }
    }
  };
  this.setMeshUvsImage = function (mesh, meshUv, objectSetting) {
    if (meshUv != null) {
      ...
      for (let faceIndex in meshUv) {
        ...
        if (materialIndex === 0) {
          let imageName = objectSetting.parameters[faceUv.imageName] == null ? "test" : objectSetting.parameters[faceUv.imageName].value;
          let imageUrl = "../../images/" + imageName;
          let texture = ImageUtils.loadTexture(imageUrl, {}, function () {});
          let faceMaterial = new MeshPhongMaterial({
            transparent: false,
  ...
  this.createNodeData = function (objectSetting, serverObjectSetting) {
    let componentInfo = thatS3dServerObjectCreator.getComponentInfo(objectSetting.code, objectSetting.versionNum);
    let parameters = {};
    for (let paramName in componentInfo.parameters) {
      let param = objectSetting.parameters[paramName];
      let comParam = componentInfo.parameters[paramName];
      if (comParam.isGeo) {
        parameters[paramName] = {
          value: serverObjectSetting.parameters[paramName].value
        };
      } else {
        parameters[paramName] = {
    ...
    if (!objectSetting.useWorldPosition) {
      objectSetting.useWorldPosition = serverObjectSetting.useWorldPosition;
    }
    objectSetting.center = serverObjectSetting.center;
    return {
      id: objectSetting.id,
  ...
  this.refreshProgress = function () {
    let succeedCount = thatS3dServerObjectCreator.creatingInfo.countInfo.serverSucceed + thatS3dServerObjectCreator.creatingInfo.countInfo.localSucceed + thatS3dServerObjectCreator.creatingInfo.countInfo.internalSucceed;
    let message = "正在调用造型服务 (" + succeedCount + " / " + thatS3dServerObjectCreator.creatingInfo.countInfo.all + ")";
    thatS3dServerObjectCreator.manager.messageBox.show({
      percent: succeedCount / thatS3dServerObjectCreator.creatingInfo.countInfo.all,
      message: message
    });
    if (succeedCount === thatS3dServerObjectCreator.creatingInfo.countInfo.all) {
      ...
      thatS3dServerObjectCreator.manager.messageBox.hide({
        timeout: 200
      });
    }
  };
  ...
  this.jsonToResourceObject3D = function (geoJson) {
    ...
    let box = new Box3().setFromObject(boxMesh, true);
  ...
  this.getMeshFromText = function (offMeshInfo, materialName, lineMaterialName, serverMaterialJsons) {
    ...
    if (offMeshInfo.indices.length > 0) {
      ...
      geometry.setAttribute("position", new Float32BufferAttribute(offMeshInfo.positions, 3));
      geometry.setIndex(new Uint16BufferAttribute(offMeshInfo.indices, 1));
    ...
    for (let i = 0; i < offMeshInfo.lines.length; i++) {
      ...
      line.isObjectLine = true;
      mesh.add(line);
    }
    return mesh;
  };
  this.getMaterial = function (materialName, serverMaterialJsons) {
    let materialInfo = thatS3dServerObjectCreator.manager.materials.getUserMaterialInfo(materialName);
  ...
  this.setMeshMaterial = function (mesh, material) {
    ...
    for (let j = 0; j < indices.length; j = j + 3) {
      let uv = [new Vector2(0, 0), new Vector2(1, 0), new Vector2(1, 1)];
      let pA = new Vector3(positions[indices[j] * 3], positions[indices[j] * 3 + 1], positions[indices[j] * 3 + 2]);
      pA.applyEuler(euler);
      let pB = new Vector3(positions[indices[j + 1] * 3], positions[indices[j + 1] * 3 + 1], positions[indices[j + 1] * 3 + 2]);
      pB.applyEuler(euler);
      let pC = new Vector3(positions[indices[j + 2] * 3], positions[indices[j + 2] * 3 + 1], positions[indices[j + 2] * 3 + 2]);
      pC.applyEuler(euler);
  ...
  //构造标注的object3Ds
  this.cloneHelperRootObject3D = function (cacheKey) {
    return thatS3dServerObjectCreator.manager.object3DCache.cloneHelperObject3D(cacheKey);
  };

  //构造标注的object3Ds
  this.createHelperRootObject3D = function (geoJson, serverMaterialJsons) {
    let helper3D = new Object3D();
    let childHelper3Ds = [];
    if (geoJson.helpers != null) {
      for (let i = 0; i < geoJson.helpers.length; i++) {
        let helperJson = geoJson.helpers[i];
        let childHelper3D = thatS3dServerObjectCreator.offToHelperObject3D(helperJson, serverMaterialJsons);
        childHelper3Ds.push(childHelper3D);
      }
    }
    for (let i = 0; i < childHelper3Ds.length; i++) {
      let childHelper3D = childHelper3Ds[i];
      helper3D.add(childHelper3D);
    }
    helper3D.isHelper = true;
    return helper3D;
  };

  //helperJson转标注object3D
  this.offToHelperObject3D = function (helperJson, serverMaterialJsons) {
    let offText = helperJson["offText"];
    let materialName = helperJson["material"];
    let lineMaterialName = helperJson["lineMaterial"];
    if (offText == null) {
      //包含子节点
      let childOffs = helperJson["children"];
      let obj3D = new Object3D();
      for (let i = 0; i < childOffs.length; i++) {
        let childOffJson = childOffs[i];
        let tempNamePath = childOffJson.name;
        thatS3dServerObjectCreator.offToSubHelperObject3D(childOffJson, helperJson, obj3D, tempNamePath, serverMaterialJsons);
      }
      return obj3D;
    } else {
      ...
      let obj3D = new Object3D();
      obj3D.add(mesh);
      //thatS3dServerObjectCreator.refreshMeshUvMaterial(mesh, helperJson);
      return obj3D;
    }
  };

  //helperJson转标注subObject3D
  this.offToSubHelperObject3D = function (helperJson, parentGeoJson, root3D, namePath, serverMaterialJsons) {
    helperJson.parentGeoJson = parentGeoJson;
    let offText = helperJson["offText"];
    let materialName = helperJson["material"];
    let lineMaterialName = helperJson["lineMaterial"];
    if (offText == null) {
      ...
      for (let i = 0; i < childOffs.length; i++) {
        let childOffJson = childOffs[i];
        let tempNamePath = (namePath.length === 0 ? "" : namePath + "/") + childOffJson.name;
        thatS3dServerObjectCreator.offToSubHelperObject3D(childOffJson, helperJson, root3D, tempNamePath, serverMaterialJsons);
      }
    } else {
      let meshInfo = thatS3dServerObjectCreator.getMeshInfoFromOff(offText);
      let mesh = thatS3dServerObjectCreator.getMeshFromText(meshInfo, materialName, lineMaterialName, serverMaterialJsons);
    ...
    helperJson.parentGeoJson = null;
  };
};

export { S3dServerObjectCreator as default };
