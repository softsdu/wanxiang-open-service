...
let S3dTree = function () {
  ...
  //隐藏
  this.hide = function () {
    $("#" + thatS3dTree.containerId).find(".s3dTreeContainer").css({
      "display": "none"
    });
  };
  ...
  this.setNodeCheckStatus = function (nodeItem, checked, changedNodeIds) {
    ...
    if (hasChildren) {
      ...
      //记录变化状态的node
      if (checked && $(checkbox).hasClass("s3dTreeNodeCheckboxNone") || !checked && $(checkbox).hasClass("s3dTreeNodeCheckboxChecked")) {
  ...
  this.initNodeJArray = function (s3dObject, id2NodeJsonMap) {
    ...
    for (let i = 0; i < s3dObject.groups.length; i++) {
      ...
      for (let j = 0; j < groupJson.objects.length; j++) {
        ...
        let oJson = s3dObject.objectMap[oId];
        groupNodeJson.objects.push({
          id: oId,
          name: oJson.name,
          isGroup: false
        });
      ...
      for (let j = 0; j < groupJson.groups.length; j++) {
        let gId = groupJson.groups[j];
        let gJson = s3dObject.groupMap[gId];
        groupNodeJson.objects.push({
          id: gId,
          name: gJson.name,
          isGroup: false
  ...
  this.getNodeHtml = function (nodeJson, expanded, isGroup) {
    ...
    nodeHtml += "<div class=\"s3dTreeNodeTitle\">" + cmnPcr.htmlEncode(nodeJson.name) + "</div>";
    nodeHtml += "</div>";
    if (isGroup) {
