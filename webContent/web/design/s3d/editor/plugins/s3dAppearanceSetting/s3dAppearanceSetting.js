...
import './s3dAppearanceSetting.css.js';
...
let S3dAppearanceSetting = function () {
  ...
  this.hide = function () {
    ...
    $(container).find(".s3dAppearanceSettingBtnSectionOuterContainer").css({
      "display": "none"
    });
    $(container).find(".s3dAppearanceSettingGroupSectionOuterContainer").css({
      "display": "none"
    });
    $(container).find(".s3dAppearanceSettingDesSectionOuterContainer").css({
      "display": "none"
    });
  };
  ...
  this.getGroupHtml = function (groupJson, checked) {
    return "<div class=\"s3dAppearanceSettingGroupContainer" + (checked ? " s3dAppearanceSettingGroupContainerActive" : "") + "\" \" groupCode=\"" + groupJson.code + "\" title=\"" + cmnPcr.htmlEncode(groupJson.name) + "\">" + "<div class=\"s3dAppearanceSettingGroupName\">" + groupJson.name + "</div>" + "</div>";
  };
  ...
  this.getDescriptionHtml = function (text) {
    return "<div class=\"s3dAppearanceSettingDesContainer\">" + "<div class=\"s3dAppearanceSettingDesText\">" + text + "</div>" + "</div>";
  };

  //获取button html
  this.getBtnHtml = function (btnJson, groupCode, checked) {
    if (btnJson.imgData == null) {
      return "<div class=\"s3dAppearanceSettingBtnContainer s3dAppearanceSettingBtnContainerOnlyText\" itemCode=\"" + btnJson.code + "\" groupCode=\"" + groupCode + "\" title=\"" + cmnPcr.htmlEncode(btnJson.name) + "\">" + "<div class=\"s3dAppearanceSettingBtnName s3dAppearanceSettingBtnNameOnlyText" + (checked ? " s3dAppearanceSettingBtnNameActive" : "") + "\">" + btnJson.name + "</div>" + "</div>";
    } else {
      if (btnJson.imgData.normal != null) {
        ...
        return "<div class=\"s3dAppearanceSettingBtnContainer\" itemCode=\"" + btnJson.code + "\" groupCode=\"" + groupCode + "\" title=\"" + cmnPcr.htmlEncode(btnJson.name) + "\">" + "<img class=\"s3dAppearanceSettingBtnImg\" src=\"" + imgUrl + "\" />" + "<div class=\"s3dAppearanceSettingBtnName" + (checked ? " s3dAppearanceSettingBtnNameActive" : "") + "\">" + btnJson.name + "</div>" + "</div>";
      } else {
        let colors = btnJson.imgData.colors;
        return "<div class=\"s3dAppearanceSettingBtnContainer\" itemCode=\"" + btnJson.code + "\" groupCode=\"" + groupCode + "\" title=\"" + cmnPcr.htmlEncode(btnJson.name) + "\">" + "<div class=\"s3dAppearanceSettingBtnColor\" style=\"background-color: #" + colors[0] + "\"></div>" + "<div class=\"s3dAppearanceSettingBtnColorAlpha1\"></div>" + "<div class=\"s3dAppearanceSettingBtnColorAlpha2\"></div>" + "<div class=\"s3dAppearanceSettingBtnName" + (checked ? " s3dAppearanceSettingBtnNameActive" : "") + "\">" + cmnPcr.htmlEncode(btnJson.name) + "</div>" + "</div>";
      }
    }
  };
...
export { S3dAppearanceSetting as default };
