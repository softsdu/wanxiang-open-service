...
import qrcode from '../../_virtual/qrcode.js';
...
let MsgBox = function () {
  ...
  this.showInfoWin = function (p) {
    let title = cmnPcr.htmlEncode(p.title);
    let info = p.isHtmlInfo ? p.info : cmnPcr.htmlEncode(p.info);
    let dialogId = cmnPcr.getRandomValue();
    let dialogLabelId = dialogId + "_label";
    let html = "<div class=\"modal fade\" style=\"z-index:200001;\" id=\"" + dialogId + "\" tabindex=\"-1\" role=\"dialog\" aria-labelledby=\"" + dialogLabelId + "\" aria-hidden=\"true\">" + "<div class=\"modal-dialog\">" + "<div class=\"modal-content\">" + "<div class=\"modal-header\">" + "<button type=\"button\" class=\"close\" data-dismiss=\"modal\" aria-hidden=\"true\">×</button>" + "<h4 class=\"modal-title\" id=\"" + dialogLabelId + "\">" + title + "</h4>" + "</div>" + "<div class=\"modal-body\">" + info + "</div>" + "<div class=\"modal-footer\">" + "<button type=\"button\" class=\"btn btn-default\" data-dismiss=\"modal\">关闭</button>" + "</div>" + "</div><!-- /.modal-content -->" + "</div><!-- /.modal-dialog -->" + "</div><!-- /.modal -->";
  ...
  this.htmlWindow = function (p) {
    ...
    if (p.type === "oneInputText") {
      ...
      let innerHtml = "<div id=\"" + titleId + "\" style=\"width:100%;height:30px;font-size:13px;text-align:center;font-weight:800;\"></div>" + "<div id=\"" + labelId + "\" style=\"witdh:100%;height:30px;font-size:11px;\"></div>" + "<div style=\"witdh:100%;height:30px;font-size:11px;text-align:center;\"><input id=\"" + textId + "\" type=\"text\" style=\"width:230px;height:20px;\" /></div>" + "<div id=\"" + buttonContainerId + "\" style=\"witdh:100%;height:30px;font-size:11px;text-align:right;\"><input type=\"button\" id=\"" + okBtnId + "\" value=\"确 定\" style=\"width:60px;height:25px;\" />&nbsp;<input type=\"button\" id=\"" + cancelBtnId + "\" value=\"取 消\" style=\"width:60px;height:25px;\" />&nbsp;</div>";
      ...
      $("#" + cancelBtnId).click(function () {
        let text = $("#" + textId).val();
        let succeed = p.cancelFunction === null || p.cancelFunction === undefined || p.cancelFunction({
          text: text
        });
        if (succeed) {
          popContainer.close();
      ...
      let okBtnId = frameId + "_ok";
      let cancelBtnId = frameId + "_cancel";
      let innerHtml = "<div id=\"" + titleId + "\" style=\"width:100%;height:30px;font-size:15px;text-align:center;font-weight:800;\"></div>" + "<div id=\"" + labelId + "\" style=\"witdh:100%;height:25px;font-size:13px;padding-left:10px;\"></div>" + "<div style=\"witdh:100%;height:140px;font-size:11px;text-align:center;\"><textarea id=\"" + textId + "\" style=\"width:390px;height:130px;resize:none;border:1px solid #95B8E7\"></textarea></div>" + "<div id=\"" + buttonContainerId + "\" style=\"witdh:100%;height:40px;font-size:11px;text-align:right;\"><input type=\"button\" id=\"" + okBtnId + "\" value=\"确 定\" style=\"width:60px;height:25px;\" />&nbsp;<input type=\"button\" id=\"" + cancelBtnId + "\" value=\"取 消\" style=\"width:60px;height:25px;\" />&nbsp;</div>";
      $("#" + popContainer.containerId).html(innerHtml);
      $("#" + titleId).text(p.title);
      $("#" + labelId).text(p.label);
      $("#" + textId).text(p.text);
...
let ServerAccess = function () {
  ...
  this.getSoapData = function (funcName, args, namespace) {
    ...
    postdata += "<soap:Body>";
    postdata += "<ns2:" + funcName + " xmlns:ns2=\"http://" + (namespace === undefined || namespace === null ? "service.platform.zlp.com" : namespace) + "/\">";
    for (let x in args) {
      postdata += "<" + x + ">" + "<![CDATA[" + args[x] + "]]>" + "</" + x + ">";
    }
    ...
    if (serverDt.rows != null) {
      for (let i = 0; i < serverDt.rows.length; i++) {
        ...
        for (let j = 0; j < serverDt.fields.length; j++) {
          ...
          let value = cmnPcr.strToObject(cmnPcr.replace(serverRow[fieldName], "\\\\\"", "\""), valueType);
...
let CommonProcessor = function () {
  this.getQRCodeHtml = function (text) {
    let qr = qrcode(0, 'L');
    qr.addData(text);
    qr.make();
  ...
  this.encodeString = function (value) {
    ...
    return typeof s != "string" ? s : s.replace(regxHmtlEncode, function ($0) {
  ...
  this.getAlertFormatString = function (type) {
    switch (type) {
      ...
      case valueType.time:
        return "请输入时间数据, 格式为'" + this.getTimeFormat() + "', 例如 2013-01-23 08:22:59";
  ...
  this.isBoolean = function (mystr) {
    ...
    return mystr === "是" || mystr === "否" || mystr === "true" || mystr === "false" || mystr === "y" || mystr === "n" || mystr === "yes" || mystr === "no";
  };
  ...
  this.strToTime = function (str) {
    if (str == null || str.length === 0) {
      ...
      if (str.indexOf("-") > -1) {
        ...
        let value = new Date(str.substr(0, 4), str.substr(4, 2) - 1, str.substr(6, 2), 0, 0, 0, 0);
        if (isNaN(value)) {
          return null;
        } else {
          return value;
      ...
      if (ss[2].indexOf(":") > -1) {
        ...
        dt = new Date(ss[0], ss[1] - 1, ss[2].substr(0, 2), stime[0], stime[1], stime[2]);
      } else {
  ...
  this.datetimeToStr = function (obj, datetimeFormat) {
    if (obj === null || obj === undefined) {
      ...
      let result = {
        'y': obj.getFullYear(),
        'M': obj.getMonth() < 9 ? "0" + (obj.getMonth() + 1) : obj.getMonth() + 1,
        'o': 1 + obj.getMonth(),
  ...
  this.arabicToChinese = function (arabic) {
    const units = ['', '十', '百', '千'];
    const bigUnits = ['', '万', '亿', '兆', '京', '垓', '秭', '穰', '沟', '涧', '正', '载'];
    const digits = '零一二三四五六七八九';
    let result = '';
    let str = arabic.toString();
    let index = 0;
    while (str.length > 0) {
      let part = '';
      let subStr = str.slice(-4);
      for (let i = 0; i < subStr.length; i++) {
        let digit = parseInt(subStr[i]);
        if (digit > 0) {
          part = digits[digit] + units[subStr.length - 1 - i] + part;
        } else if (part.length > 0 && part[part.length - 1] !== digits[0]) {
          part = digits[0] + part;
        }
      }
      if (part.length > 0) {
    ...
    if (obj === undefined || obj === null) {
      ...
      switch (type) {
        ...
        case valueType.date:
          return this.datetimeToStr(obj, "yyyy-MM-dd");
        default:
          alert("objectToShowStr, type=" + type + ", obj=" + obj.toString());
          return null;
      }
    }
  };
  ...
  //color转单个值数值 added by ls 20221202
  this.colorToRGBIntArray = function (colorObject) {
    let intObj = {};
    colorObject.getRGB(intObj);
    return [intObj.r, intObj.g, intObj.b];
  };

  //color转单个值数值 added by ls 20221202
  this.colorToString = function (colorObject) {
    return "#" + colorObject.getHexString();
  };
  ...
  //获取网页地址中的参数
  this.getQueryArgsFromUrl = function (url) {
    let urlSplitterIndex = url.lastIndexOf("?");
    if (urlSplitterIndex !== -1) {
      let query = url.substring(urlSplitterIndex + 1); // 取查询字符串
      return this.getQueryArgsFromString(query);
    } else {
      return {};
    }
  ...
  this.getSortedWebFonts = function () {
    let fonts = s3dWebFonts;
    let sortedFonts = [];
    for (let i = 0; i < fonts.length; i++) {
      let font = fonts[i];
      let tempFonts = [];
      let added = false;
      for (let j = 0; j < sortedFonts.length; j++) {
        let tempFont = sortedFonts[j];
        if (tempFont.name.localeCompare(font.name) > 0 && !added) {
          tempFonts.push(font);
          added = true;
        }
        tempFonts.push(tempFont);
      }
      if (!added) {
        tempFonts.push(font);
      }
      sortedFonts = tempFonts;
    }
    return sortedFonts;
  };
};
let PopupContainer = function (p) {
  ...
  this.contentId = cmnPcr.getRandomValue();
  ...
  this.getWindowOffset = function () {
    let ww = 0,
      wh = 0;
    // 获取窗口宽度
    if (window.innerWidth) ww = window.innerWidth;else if (document.body && document.documentElement.clientWidth) ww = document.documentElement.clientWidth;
    // 获取窗口高度 
    if (window.innerHeight) wh = window.innerHeight;else if (document.body && document.documentElement.clientHeight) wh = document.documentElement.clientHeight;
    return {
      width: ww,
      height: wh
    };
  ...
  this.calculatePos = function (winSize, popSize) {
    ...
    if (top === undefined) {
      if (winSize.height >= popSize.height) top = (winSize.height - popSize.height) / 2;else {
        top = 0;
      }
    }
    if (left === undefined) {
      if (winSize.width > popSize.width) left = (winSize.width - popSize.width) / 2;else {
        left = 0;
      }
    }
    return {
  ...
  this.show = function (callbackFunc) {
    if (thatPopContainer.isHidden) {
      ...
      //alert(popSize.width)
      let popHtml = "<div class=\"s3dPopOverlay\" id=\"" + thatPopContainer.overlayId + "\" style=\"z-index:" + overlayZIndex + ";\">" + "<div class=\"s3dPopGround\"></div>" + "<div class=\"s3dPopBox\" id=\"" + thatPopContainer.boxId + "\" style=\"z-index:" + boxZIndex + ";left:" + newPos.left + "px;top:" + thatPopContainer.top + "px;width:" + popSize.width + "px;height:" + popSize.height + "px;\">" + "<div class=\"s3dPopTitle\"></div><div class=\"s3dPopCloseBtn\">×</div>" + "<div class=\"s3dPopInnerBox\" id=\"" + thatPopContainer.contentId + "\"></div>" + "</div>" + "</div>";
      $("#" + thatPopContainer.containerId).append(popHtml);
      let box = $("#" + thatPopContainer.boxId);
      let overlay = $("#" + thatPopContainer.overlayId);
      $(box).find(".s3dPopTitle").text(thatPopContainer.title);
      $(box).find(".s3dPopCloseBtn").click(function () {
      ...
      $(box).find(".s3dPopTitle").mousedown(function (e) {
        let dv = $(this).parent();
      ...
      $(overlay).mousemove(function (e) {
        if (thatPopContainer.dragInfo.isDown) {
          let x = e.pageX - thatPopContainer.dragInfo.distanceX;
          let y = e.pageY - thatPopContainer.dragInfo.distanceY;
          if (x < 0) {
            x = 0;
...
//一般通用处理
const cmnPcr = new CommonProcessor();
...
const s3dAnimationEditType = {
  ...
  editKeyFrameIndex: "editKeyFrameIndex",
  editKeyFrameValue: "editKeyFrameValue",
  editKeyFrameLineType: "editKeyFrameLineType"
};
const s3dContent2DEditType = {
  add: "add",
  delete: "delete",
  edit: "edit",
  editName: "editName"
};
const s3dProgressEditType = {
  add: "add",
  delete: "delete",
  edit: "edit",
  editName: "editName",
...
const s3dMaterialEditType = {
  add: "add",
  delete: "delete",
  edit: "edit",
  editProperty: "editProperty"
};
const s3dOperateTypeName = {
  transform: "移动旋转缩放",
  changeParent: "更改结构",
  edit: "编辑",
  add: "添加",
...
const s3dResourceType = {
  gltf: "gltf",
  fbx: "fbx"
};
const s3dElement3DType = {
  group: "group",
  unit: "unit",
  camera: "camera",
  light: "light",
...
const s3dAnimationGroupType = {
  property: "property",
  material: "material"
};
const s3dProgressGroupType = {
  property: "property",
  material: "material"
};
const s3dTransformMode = {
  rotation: "rotate",
  position: "position",
  scale: "scale"
};
...
const s3dClockDirection = {
  clockwise: "clockwise",
  anticlockwise: "anticlockwise"
};
const s3dContent2DShiftType = {
  fadeInOut: "fadeInOut",
  horizontal: "horizontal",
  vertical: "vertical"
};
const s3dContent2DShiftName = {
  fadeInOut: "淡入淡出",
  horizontal: "水平滑入",
  vertical: "竖直滑入"
};
const s3dImageSourceType = {
  object: "object",
  local: "local",
  system: "system"
};
const s3dTextDirections = [{
  code: "Horizontal",
  name: "横向"
}, {
  code: "Vertical",
...
*/
{
  ...
  code: "Source Han Sans",
  name: "Source Han Sans"
}, {
  code: "Source Han Serif",
...
export { CommonProcessor, MsgBox, PopupContainer, ServerAccess, WaitingBar, cmnPcr, msgBox, paramType, s3dAnimationEditType, s3dAnimationGroupType, s3dClockDirection, s3dContent2DEditType, s3dContent2DShiftName, s3dContent2DShiftType, s3dElement3DType, s3dImageSourceType, s3dLayerType, s3dMaterialEditType, s3dNormalViewport, s3dOperateType, s3dOperateTypeName, s3dProgressEditType, s3dProgressGroupType, s3dResourceType, s3dTextDirections, s3dTransformMode, s3dUiStatus, s3dUiStatusText, s3dViewLevel, s3dWebFonts, serverAccess, valueType };
