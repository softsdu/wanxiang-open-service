...
let S3dMessageBox = function () {
  ...
  this.show = function (p) {
    ...
    $(container).find(".s3dMessageBoxContainer").css({
      display: "block"
    });
  ...
  this.refreshText = function (p) {
    let container = $("#" + thatS3dMessageBox.containerId);
    let innerContainer = $(container).find(".s3dMessageBoxInnerContainer");
    let textBox = $(container).find(".s3dMessageBoxText");
    let progressWidth = p.percent * $(innerContainer).width();
    $(textBox).width(progressWidth);
  ...
  this.getWinHtml = function () {
    return "<div class=\"s3dMessageBoxContainer\">" + "<div class=\"s3dMessageBoxInnerContainer\">" + "<div class=\"s3dMessageBoxBackground\"></div>" + "<div class=\"s3dMessageBoxText\"></div>" + "</div>" + "</div>";
  };
};

export { S3dMessageBox as default };
