...
window.common3DFunction = {
   ...
   */
  //按照比例转换（显示值v转为存储值s)
  v2s: function (mm, ratio) {
    return mm / ratio;
  },
  //按照比例转换（存储值s转为显示值v)
  s2v: function (m, ratio) {
    return m * ratio;
  },
  //角度转弧度
  ...
  checkSamePoint: function (pointA, pointB, ignoreSize) {
    if (ignoreSize == null) {
      ...
      return Math.abs(pointA.x - pointB.x) <= ignoreSize && Math.abs(pointA.y - pointB.y) <= ignoreSize && Math.abs(pointA.z - pointB.z) <= ignoreSize;
    }
  }
};
...
window.commonRotate = {
  ...
  rotatePoint2D: function (point, degress) {
    if (point.x === 0 && point.y === 0) {
      ...
      var v = new Vector3(point.x, 0, point.y);
      var p = v.applyAxisAngle(new Vector3(0, 1, 0), degress);
      return {
        x: p.x,
        y: p.z
      };
...
window.commonLine = {
  ...
  onSegment2D: function (pointA, pointB, point2D) {
    if ((point2D.x - pointA.x) * (pointB.y - pointA.y) == (pointB.x - pointA.x) * (point2D.y - pointA.y) && cmnPcr.min([pointA.x, pointB.x]) <= point2D.x && point2D.x <= cmnPcr.max([pointA.x, pointB.x]) && cmnPcr.min([pointA.y, pointB.y]) <= point2D.y && point2D.y <= cmnPcr.max([pointA.y, pointB.y])) {
      return true;
    } else {
      return false;
    }
