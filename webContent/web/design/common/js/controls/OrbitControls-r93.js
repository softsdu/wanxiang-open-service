...
//如果启用自动旋转，则必须在动画循环中调用controls.update（）
	...
	//最新的设置 modified by ls 20210415
	//this.mouseButtons = { ORBIT: null, ZOOM: THREE.MOUSE.MIDDLE, PAN: THREE.MOUSE.RIGHT };
	
	//更改鼠标键的设置 modified by ls 20221020
	...
	function onMouseDown( event ) {
		...
		switch ( event.button ) {
			...
			case scope.mouseButtons.ORBIT:
				...
				break; 
				/*取消zoom modified by ls 20221020
			...
			case scope.mouseButtons.PAN:
				...
				//如果按下的shift，那么旋转，否则平移 modified by ls 20221020
				if(event.shiftKey){
					if ( scope.enableRotate === false ) return;
					handleMouseDownRotate( event );
					state = STATE.ROTATE;
