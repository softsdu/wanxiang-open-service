...
class OrbitControlsGizmo {
    ...
    function drawLayers ( clear ) {
      ...
      for(let i = 0, length = axes.length; i < length; i ++) {
        ...
        drawCircle(axis.position, axis.size, highlight ? "#444444" : color);
        ...
        if (axis.label) {
          ...
          context.fillText(axis.label, axis.position.x, axis.position.y + 1);
        }
      }
    }
...
export default OrbitControlsGizmo;