//导出S3Dc文件（纯文本）
js3CommandProcessors["exportS3Dc"] = {
	toStatus: "normal",	
	icon: "/images/s3dc.png",
	editor: null,
	popContainer: null,
	...
	publishInfo: null,
	run: function(p){
		let commandProcessor = js3CommandProcessors["exportS3Dc"];
	...
	showDownloadWindow: function(){
		...
		let innerHtml = "<div style='position:absolute;left:0px;right:0px;top:0px;bottom:40px;color:#444444;padding-top:10px;line-height:40px;font-size:15px;text-align:center;'>"
			+ "<span style='color:#444444;text-align:center;'>请下载S3Dc文件:</span>"
			+ "<br/>"
			+ "<a name='downloadBtn' style='border:solid 1px #f0f0f0;padding-left:20px;padding-right:20px;padding-top:10px;padding-bottom:10px;border-radius:3px;background-color:#337ab7;color:#ffffff;text-align:center;cursor:pointer;'>" + commandProcessor.sourceComponentInfo.name + ".s3dc</a>"
			+ "</div>"
		...
		$(container).find("a[name='downloadBtn']").click(function(e){
			let commandProcessor = js3CommandProcessors["exportS3Dc"];
			let text = JSON.stringify(commandProcessor.publishInfo);
			let blob = new Blob([text], { type: "text/plain"});
			let link = document.createElement("a");
			link.style.display = "none";
			link.href = window.URL.createObjectURL(blob);
			link.download = commandProcessor.publishInfo.name + ".s3dc";
			link.click();
			return false;
		});
	...
	showExplodeWindow: function (){
		...
		popContainer.show();
		let explodeUrl = basePath + "/web/design/common/plugins/exportS3Dc/explodeCom/explodeEditor.jsp";
		...
		$(container).find("input[name='okBtn']").click(function(){
			...
			explodePageWindow.comEditor.generateS3dc();
			commandProcessor.checkGenerateS3dcStatus();
		});
		$(container).find("input[name='cancelBtn']").click(function(){
			let commandProcessor = js3CommandProcessors["exportS3Dc"];
			commandProcessor.popContainer.close();
	...
	//检测是否已生成S3D文件
	checkGenerateS3dcStatus: function(){
		setTimeout(function(){
			let commandProcessor = js3CommandProcessors["exportS3Dc"];
			let container = $("#" + commandProcessor.popContainer.containerId);
			let explodePageWindow = $(container).find("iframe[name='innerPage']")[0].contentWindow;
	...
	showComponentUnitTree: function(allSubComponentInfoHash){
		...
		popContainer.show();
		let treeConfigUrl = basePath + "/web/design/common/plugins/exportS3Dc/comUnitTree/comUnitTree.jsp";
		let innerHtml = "<div style='position:absolute;left:0px;right:0px;top:0px;bottom:40px;font-size:15px;text-align:center;'>"
