//编辑pathClosed2d added by ls 20230109
js3CommandProcessors["pathClosed2d"] = {
	toStatus: "normal",	
	editor: null,
	intervalId: null,
	paramName: null,
	...
	run: function(p){
		var thatCommandJson = js3CommandProcessors["pathClosed2d"];
		...
		thatCommandJson.showLayoutPath2dWindow(p); 
	},
	showLayoutPath2dWindow: function(p){ 
		...
		js3CommandProcessors["pathClosed2d"].intervalId = setInterval(function(){
			var contentWindow = $("#" + editorFrameId)[0].contentWindow;
			if(contentWindow != null){
				var thatCommandJson = js3CommandProcessors["pathClosed2d"];
				var layoutPaht2dInfo = thatCommandJson.paramValue == null || thatCommandJson.paramValue.length == 0 ? {} : cmnPcr.strToJson(thatCommandJson.paramValue);
				contentWindow.layoutInfo = layoutPaht2dInfo;
				window.clearInterval(thatCommandJson.intervalId);
			}
		},50)
		
		$("#" + editorFrameId).attr("src", "../layoutPath2d/layoutPath2dEditor.jsp");
		$("#" + okBtnId).click(function(){
			...
			if(layoutInfo.paths.length == 0){
				...
				if(!layoutInfo.paths[0].isClosed){
					msgBox.alert({info: "路径必须闭合."});
				}
				else{
			...
			if(isValidatedValue){
				...
				unitComponentProcessor.afterEditPath2d({
					paramName: paramName,
