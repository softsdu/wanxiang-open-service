...
js3CommandProcessors["replaceRelatedUnit"] = {
	toStatus: "none",
	icon: "/images/replaceRelatedUnit.png",
	editor: null,
	popContainer: null,
	mapTypeCode: "replace",
	allUnitIds: null,
	...
	showComponentListDialog: function(p){
		...
		let popContainer = new PopupContainer({
			width: 380,
			height: (relatedMapComs.length > 4 ? 4 : relatedMapComs.length) * 60 + 110 + 40,
			top: 50,
			title: "选择关系构件"
		});
		popContainer.show();
		...
		let innerHtml = "<div style=\"position:absolute;left:0px;right:0px;top:0px;bottom:80px;font-size:11px;text-align:center;overflow-y:hidden;\">"
		...
		$(container).find("iframe[name='editorFrame']").attr("src", "../common/plugins/replaceRelatedUnit/replaceRelatedUnit.jsp?code=" + p.code + "&versionNum=" + p.versionNum + "&mapTypeCode=" + p.mapTypeCode);
		$(container).find(".commonBtn[name='okOneBtn']").click(function(){
			let commandJson = js3CommandProcessors["replaceRelatedUnit"];
	...
	replaceUnits: function(p){
		...
		for(let i = 0; i < unitIds.length; i++){
			...
			let groupId = editor.getUnitGroupId(unitData.id);
			
			for(let j = 0; j < mapCom.properties.length; j++){
				let property = mapCom.properties[j];
			...
			let unitSetting = {
				...
				uvs: null,
				otherInfo: {
					//和fromObject3D放在同一个组里
					groupId: groupId,
						
					//如果是替换多个，那就不选中了
					needSelect: unitIds.length === 1,
					
					commandName: "replaceRelatedUnit",
	...
	updateToObjectRotationAndPosition: function(toObject3D){
		...
		//删除原始构件
		editor.removeUnitObject3D(fromObject3D, true);
	},
