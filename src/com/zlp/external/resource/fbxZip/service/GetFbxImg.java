...
public class GetFbxImg extends HttpServlet{
    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {  
        ...
        try { 
            ...
            String imgName = this.getParameterValue(request, "imgName");
			String resFbxName = this.getParameterValue(request, "resFbxName");
			if(resFbxName == null){
				String resFbxId = this.getParameterValue(request, "resFbxId");
				byte[] data = this.getResDataById(resourceFileProcessor, resFbxId, imgName);
				this.writeResponse(resourceFileProcessor, response, data);
			}
			else {
				byte[] data = this.getResDataByName(resourceFileProcessor, resFbxName, imgName);
				this.writeResponse(resourceFileProcessor, response, data);
			}
        }
        catch (Exception e) { 
	...
	private byte[] getResDataByName(IFbxZipProcessor resourceFileProcessor, String resFbxName, String imgName) throws Exception{
		...
		try {
			String imgFilePath = resourceFileProcessor.getFbxImgFilePathByName(resFbxName, imgName);
	...
	private byte[] getResDataById(IFbxZipProcessor resourceFileProcessor, String resFbxId, String imgName) throws Exception{
		FileInputStream fis = null;
		try {
			String imgFilePath = resourceFileProcessor.getFbxImgFilePathById(resFbxId, imgName);
